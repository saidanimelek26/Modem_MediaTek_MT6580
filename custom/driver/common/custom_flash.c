...
#if defined(__UBL__)    // Bootloader (including mini and extended bootloader)

    #if defined(__NAND_DRV_IN_BL__)
        #define __DISK_DRV_REQUIRED__
        #ifndef _NAND_FLASH_BOOTING_
		        #define _NAND_FLASH_BOOTING_
	      #endif
    #endif

    #if defined(__SF_DRV_IN_BL__) || defined(__EMMC_DRV_IN_BL__) || defined(__NOR_DRV_IN_BL__)
        #define __DISK_DRV_REQUIRED__
    #endif

#else   // main load and FOTA (FOTA does not define __UBL__)

    #if defined(__SMART_PHONE_PLATFORM__)        // SMART_PHONE_CORE != NONE
        #ifndef __MAUI_DISK_DRV_NOT_REQUIRED__       // SMART_PHONE_CORE = ANDROID_MODEM or =MODEM_HOST. Need no maui driver. 
            #define __DISK_DRV_REQUIRED__
        #endif
    #else                                   // SMART_PHONE_CORE = NONE
        #define __DISK_DRV_REQUIRED__
    #endif
