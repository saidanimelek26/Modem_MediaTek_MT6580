...
{
    /*
    * Please don't switch this return to KAL_TRUE since we don't support this item currently, 
    * if you have requirement about this item, please contact with SIM owner
    */
...
kal_uint8 custom_sat_additional_lock_switch(void)
{
     ...
     * 0: turn off
     */
#if defined(__SAT_ADDITIONAL_LOCK__) || defined(__PPE_INTERFACE_SUPPORT__)
    return 1;   
#else
    return 0;
#endif
...
kal_bool custom_sat_clear_loci_on_sat_refresh(sat_refresh_type_enum refresh_type)
{
    ...
    switch (refresh_type)
    {
        ...
        case TYPE_SIM_RESET:
            /* No Impact, controlled by compilation switch __DUAL_IMSI__ */
            result = KAL_FALSE;
            break;                            
...
/*****************************************************************************
* FUNCTION
*  custom_sat_ussd_do_not_inform_user_when_alpha_id_present
* DESCRIPTION
...
* While some SIM card may use DISPLAY_TEXT to tell user the SAT USSD result even when
* "alpha_id is present and no NULL object". 
* In this case, there might be 2 dialog telling the USSD result : one from network, another
* one from SAT DISPLAY_TEXT.
*
...
*  KAL_FALSE or KAL_TRUE
*****************************************************************************/
kal_bool custom_sat_ussd_do_not_inform_user_when_alpha_id_present(const kal_char *mcc_mnc_str)
{
...
kal_uint32 sml_seconds_to_expire = 0;
extern kal_bool smu_sml_timer_expiry_callback(void *param);
 ...
 *  void
*******************************************************************************/
void sml_printkey(t_cust_chl_sym_key *key)
{
#if 0
/* under construction !*/
/* under construction !*/
                           ...
                           kal_uint8 source)
{
    ...
    switch(item)
    {
        case SML_CAT_VERSION:
            *plength = SML_SIZE_OF_VERSION;
            return (kal_uint32 *) &(pObj->version[0]);
        
        case SML_CAT_PERTECTION_SCHEME:
        ...
        case SML_CAT_LENGTH:
            *plength = SML_SIZE_OF_LENGTH;
            return (kal_uint16 *) &(pObj->length[0]);
        
        case SML_CAT_NUM_PLMN:
            *plength = SML_SIZE_OF_NUM_PLMN;
            return (kal_uint16 *) &(pObj->num_of_plmn[0]);

        case SML_CAT_PLMN_LIST:
            *plength = SML_SIZE_OF_PLMN_LIST;
            return (kal_uint8 *) &(pObj->plmn_list[0]);

        case SML_CAT_NUM_GID:
            *plength = SML_SIZE_OF_NUM_GID;
            return (kal_uint16 *) &(pObj->num_of_gid[0]);

        case SML_CAT_GID_LIST:
            *plength = SML_SIZE_OF_GID_LIST;
            return (kal_uint8 *) &(pObj->gid_list[0]);

        case SML_CAT_START_TIME:
            *plength = SML_SIZE_OF_START_TIME;
            return (kal_uint32 *) &(pObj->start_time[0]);
            
        case SML_CAT_END_TIME:
            *plength = SML_SIZE_OF_END_TIME;
            return (kal_uint32 *) &(pObj->end_time[0]);

        case SML_CAT_IMEI:
            *plength = SML_SIZE_OF_IMEI;
            return (kal_uint8 *) &(pObj->imei[0]);

        case SML_CAT_VERIFICATION_CODE:
            *plength = SML_SIZE_OF_VERIFICATION_CODE;
            return (kal_uint8 *) &(pObj->verification_code[0]);

        case SML_CAT_UNLOCK_TIME_COUNTER:
            *plength = SML_SIZE_OF_UNLOCK_TIME;
 ...
 *  SMLOBJ
 *******************************************************************************/ 
kal_bool sml_checkValidity(void *pObj)
{
    sml_context_struct *pSmlObj = (sml_context_struct *) pObj;
    ...
    if (pSmlObj->protection_scheme == SML_PROTECTION_SCHEME_HMAC_SHA256)
    {
        ...
        {
            sml_printkey(&key);
            sml_Dump("verify mac", &(pSmlObj->verification_code[0]), SML_SIZE_OF_MAC);
 ...
 *  SMLOBJ
 *******************************************************************************/ 
kal_uint32 sml_nwTimeToSeconds(kal_uint8 nw_time_zone, nw_time_zone_time_struct *nw_time)
{
...
/*******************************************************************************
 * FUNCTION  
 *  sml_UpdateNwTimeAndCheckTempLock
 * DESCRIPTION
 ...
 *  SMLOBJ
 *******************************************************************************/ 
kal_bool sml_UpdateNwTimeAndCheckTempLock(kal_uint8 source, kal_uint8 nw_time_zone, nw_time_zone_time_struct *nw_time)
{
    ...
    state = (kal_uint8 *)(*p->getItem)(SML_CAT_C, SML_CAT_STATE, &length, source);
    start_time = (kal_uint8 *)(*p->getItem)(SML_CAT_C, SML_CAT_START_TIME, &length, source);
    end_time = (kal_uint8 *)(*p->getItem)(SML_CAT_C, SML_CAT_END_TIME, &length, source);

    if (*state != SML_STATE_TEMPORARY_UNLOCK)
    {
        return KAL_FALSE;
    }
    ...
    start_seconds = 0;
    for (i = 0; i < SML_SIZE_OF_START_TIME-1; i++)
    {
        start_seconds += start_time[i];
        start_seconds = (start_seconds << 8);
    }
    start_seconds += start_time[SML_SIZE_OF_START_TIME-1];
    start_seconds /= 1000;  /* from milliseconds to seconds */

    start_seconds_u32 = (kal_uint32)start_seconds;

    duration_seconds = 0;
    for (i = 0; i < SML_SIZE_OF_END_TIME-1; i++)
    {
        duration_seconds += end_time[i];
        duration_seconds = (duration_seconds << 8);
    }
    duration_seconds += end_time[SML_SIZE_OF_END_TIME-1];

    /* calculate current time in seconds */
    current_seconds = sml_nwTimeToSeconds(nw_time_zone, nw_time);

    kal_brief_trace(TRACE_INFO, INFO_SML_TEMP_UNLOCK_TIME, start_seconds_u32, duration_seconds, current_seconds);
    ...
    {
        kal_brief_trace(TRACE_INFO, INFO_SML_NW_TIME_EXPIRED);
        /* expired */
        //*state = SML_STATE_PERMANENT_LOCK;
        //sml_seconds_to_expire = 0;       
        return KAL_TRUE;
    }
    else
    {
        /* update unlock time for power off accuracy: start ---> unlock time ---> NITZ ---> power off/on ---> end */
        sml_seconds_to_expire = current_seconds - start_seconds_u32;
        (*p->putItem)(SML_CAT_C, SML_CAT_UNLOCK_TIME_COUNTER, &sml_seconds_to_expire, &length, source);
        smu_nvram_write(NVRAM_EF_SML_LID, SML_NVRAM_ACCESS_ID_UNLOCK_EXPIRE);
 ...
 *  SMLOBJ
 *******************************************************************************/ 
kal_bool sml_CheckTempUnlock(kal_uint8 source)
{
    nvram_ef_sml_obj_struct *p = &pSMLg[source];    
    ...
    kal_brief_trace(TRACE_INFO, INFO_SML_CHECK_TEMP_UNLOCK, sml_seconds_to_expire, duration_seconds);

    //kal_prompt_trace(MOD_SMU, "counter: %d duration_seconds:%d", 
    //    sml_seconds_to_expire + (SML_TIMEOUT_PERIODIC_CHECK/KAL_TICKS_1_SEC),duration_seconds);

    if ((sml_seconds_to_expire + (SML_TIMEOUT_PERIODIC_CHECK/KAL_TICKS_1_SEC)) >= duration_seconds)
    {
        *state = SML_STATE_PERMANENT_LOCK;
    ...
    {
        sml_seconds_to_expire += (SML_TIMEOUT_PERIODIC_CHECK/KAL_TICKS_1_SEC);
        (*p->putItem)(SML_CAT_C, SML_CAT_UNLOCK_TIME_COUNTER, &sml_seconds_to_expire, &length, source);
 ...
 *  SMLOBJ
 *******************************************************************************/ 
kal_bool sml_FirstLoad( void *pLid, kal_uint8 source )
{
    nvram_ef_sml_obj_struct *p = &pSMLg[source];
    kal_uint32 i;
    ...
    kal_uint8 imei_ascii[SML_SIZE_OF_IMEI];

    pObj = (sml_context_struct *) pLid;

    /* the verification code will be all zero if it is the first time to load */
    for (i = 0; i < SML_SIZE_OF_VERIFICATION_CODE; i++)
    {
    ...
    if (isFirstLoad == KAL_TRUE)
    {
        ...
        if (nvram_get_imei_value(8, imei_bcd, (1 + source)) == KAL_TRUE)
        {
            ...
            kal_mem_cpy(&(pObj->imei[0]), imei_ascii, SML_SIZE_OF_IMEI);
        }
        ...
        if (pObj->protection_scheme == SML_PROTECTION_SCHEME_HMAC_SHA256)
        {
            ...
            if (ret == CUST_CHL_ERROR_NONE)
            {
                ...
                if (ret == CUST_CHL_ERROR_NONE)
                {
                    ...
                    // Test validation
                    if (sml_checkValidity((void*)pObj) == KAL_FALSE)
                    {
                        kal_brief_trace(TRACE_WARNING, WARNING_SML_LOAD_CHECK_VALIDITY_FAILED);
                    }
                }
            }
        }
 ...
 *  SMLOBJ
 *******************************************************************************/
kal_uint8 *sml_ReconstructBlob( sml_context_struct *pObj, kal_uint32 *pLen )
{
    kal_uint8 *pData = NULL;
    ...
    sizeN = sml_GetCount(&(pObj->num_of_plmn[0])) * SML_SIZE_OF_MCC_MNC;
    sizeM = sml_GetCount(&(pObj->num_of_gid[0])) * SML_SIZE_OF_GID;

    if (pObj->protection_scheme == SML_PROTECTION_SCHEME_HMAC_SHA256)
    {
        data_len = blob_len - SML_SIZE_OF_MAC;
    }
    else if (pObj->protection_scheme == SML_PROTECTION_SCHEME_RSA2048)
    {
        data_len = blob_len - SML_SIZE_OF_SIGNATURE;
    }
    else
    {
        data_len = 0;
    ...
    *pLen = data_len;
    ...
    len = SML_SIZE_OF_VERSION + 1 + 1 + SML_SIZE_OF_LENGTH + SML_SIZE_OF_NUM_PLMN + sizeN;
    kal_mem_cpy(pData, (kal_uint8 *)pObj, len);

    // num of gid + gid list size
    idx = len;
    len = SML_SIZE_OF_NUM_GID + sizeM;
...
kal_uint16 sml_GetCount( kal_uint8 *pData )
{
    ...
    kal_brief_trace(TRACE_INFO, INFO_SMU_GET_COUNT, pData[0], pData[1], num);
...
kal_uint16 sml_Load( void *pLid, kal_uint8 source )
{
    ...
    if (sml_checkValidity(pLid) == KAL_FALSE)
    {
        kal_brief_trace(TRACE_WARNING, WARNING_SML_LOAD_CHECK_VALIDITY_FAILED);
        length = SML_INVALID_CONTEXT_OBJ_LENGTH;
    }
                    ...
                    kal_uint8 source)
{
    ...
    kal_uint16 length = 0;
    kal_uint8 code[SML_SIZE_OF_MCC_MNC_GID] = {0};
    kal_uint8 code_len = 0;
    kal_uint8 compare_length = 0;
    kal_uint16 num_plmn, num_gid;
    kal_uint16 i, j;
    kal_uint8 custom_exception[3][SML_SIZE_OF_MCC_MNC] = {{0x31, 0x03, 0x10, 0x00}, 
                                                          {0x31, 0x03, 0x00, 0x00}, 
                                                          {0x00, 0x10, 0x10, 0x00}};

    /* Check the integration of SIM Lock blob first */
    if ((p == NULL) || (p != NULL && sml_checkValidity(p->pObj) == KAL_FALSE))
    {
        kal_brief_trace(TRACE_WARNING, WARNING_SML_CHECK_VALIDITY_FAILED);
        return KAL_FALSE;
    }
    
    pState = (kal_uint8 *)(*p->getItem)(cat, SML_CAT_STATE, &length, source);

    *remain = 0xFF;    /* TMO SIM lock doesn't need retry counter */

    if (*pState == SML_STATE_TEMPORARY_UNLOCK)
    {
        kal_uint8 *counter = NULL;
        kal_uint32 time_in_sec = 0;
        kal_uint8 i;

        counter = (kal_uint8 *)(*p->getItem)(cat, SML_CAT_UNLOCK_TIME_COUNTER, &length, source);
        
        for (i = 0; i < (SML_SIZE_OF_UNLOCK_TIME - 1); i++)
        {
            time_in_sec += counter[(SML_SIZE_OF_UNLOCK_TIME-1-i)];
            time_in_sec = (time_in_sec << 8);
        }
        time_in_sec += counter[0];

        sml_seconds_to_expire += time_in_sec;

        if (smu_sml_timer_expiry_callback(NULL) == KAL_TRUE)
        {
            // Time expired -> back to LOCK , reset counter
            *pState = SML_STATE_PERMANENT_LOCK;
            //return KAL_FALSE;
        }
        else
        {
    ...
    if (*pState == SML_STATE_PERMANENT_UNLOCK)
    {
        /* always pass if it is not locked! */
        return KAL_TRUE;
    }

    if (*pState == SML_STATE_PERMANENT_LOCK)
    {
        code_len = sml_GetCode(cat, imsi, gid1, gid2, sim_mnc_len, NULL, code);
        compare_length = SML_SIZE_OF_MCC_MNC + SML_SIZE_OF_GID;

        num_gid = sml_GetCount(((*p->getItem)(cat, SML_CAT_NUM_GID, &length, source)));
        pData_gid = (kal_uint8 *)(*p->getItem)(cat, SML_CAT_GID_LIST, &length, source);
    }
    else /* SML_STATE_PARTIAL_UNLOCK */
    {
        /* GID is not necessary in partial unlock */
        code_len = sml_GetCode(cat, imsi, NULL, NULL, sim_mnc_len, NULL, code);
        compare_length = SML_SIZE_OF_MCC_MNC;       
    }
    ...
    num_plmn = sml_GetCount(((*p->getItem)(cat, SML_CAT_NUM_PLMN, &length, source)));
    pData_plmn = (kal_uint8 *)(*p->getItem)(cat, SML_CAT_PLMN_LIST, &length, source);    

    kal_brief_trace(TRACE_INFO, INFO_SMU_CHECK_LENGTH, code_len, compare_length, sim_mnc_len);

    sml_Dump("code", code, SML_SIZE_OF_MCC_MNC_GID);
    sml_Dump("plmn list", pData_plmn, (SML_SIZE_OF_MCC_MNC*num_plmn));
    sml_Dump("gid1 list", pData_gid, (SML_SIZE_OF_GID*num_gid));
    sml_Dump("ignore list", custom_exception, (3*SML_SIZE_OF_MCC_MNC));

    for (i = 0; i < num_plmn; i++)
    {        
        if (kal_mem_cmp(code, (pData_plmn + (i*SML_SIZE_OF_MCC_MNC)), SML_SIZE_OF_MCC_MNC) == 0)
        {
            if (compare_length == SML_SIZE_OF_MCC_MNC)   // partial
            {
                return KAL_TRUE;
            }
            else
    {
                ...
                for (j = 0; j < 3; j++)
                {
                    if (kal_mem_cmp(code, &custom_exception[j], SML_SIZE_OF_MCC_MNC) == 0)
                    {
                        kal_brief_trace(TRACE_INFO, INFO_SMU_CHECK_IGNORE_PID1,
                ...
                for (j = 0; j < num_gid; j++)
                {
                    if (kal_mem_cmp(&code[SML_SIZE_OF_MCC_MNC], (pData_gid + (j*SML_SIZE_OF_GID)), SML_SIZE_OF_GID) == 0)
    {
        return KAL_TRUE;
    }
                }
                       ...
                       kal_uint8 *code )
{
    ...
    if (gid1 != NULL)
    {
        ...
        code[5] = gid1[1];
        //code[5] = gid2[0];
        code_len = SML_SIZE_OF_MCC_MNC_GID;
    }
    else
    {
        code_len = SML_SIZE_OF_MCC_MNC;
    }
 ...
 *  none
 *******************************************************************************/ 
void sml_GetVersion(kal_uint8 *version, kal_uint8 source)
{
    nvram_ef_sml_obj_struct *p = &pSMLg[source];
    kal_uint16 length;
    kal_uint8 *slb_ver;

    slb_ver = (kal_uint8 *) (*p->getItem)(SML_CAT_C, SML_CAT_VERSION, &length, source);

    kal_mem_cpy(version, slb_ver, SML_SIZE_OF_VERSION);
}
...
void sml_GetLockState(kal_uint8 *lock_state, kal_uint8 source)
{
    ...
    slb_state = (kal_uint8 *)(*p->getItem)(SML_CAT_C, SML_CAT_STATE, &length, source);
...
kal_uint32 sml_GetUnlockTimeLeft(kal_uint8 source)
{
    ...
    slb_end_time = (kal_uint8 *)(*p->getItem)(SML_CAT_C, SML_CAT_END_TIME, &length, source);
    ...
    duration_seconds += slb_end_time[SML_SIZE_OF_END_TIME-1];

    kal_brief_trace(TRACE_INFO, INFO_SMU_GET_UNLOCK_TIME_LEFT, duration_seconds, sml_seconds_to_expire);

    if (sml_seconds_to_expire >= duration_seconds)
    {
        return 0;
    }
    else
    {
        return (duration_seconds - sml_seconds_to_expire);
    }
}


#else /* #ifdef __TMO_SIM_LOCK__ */
