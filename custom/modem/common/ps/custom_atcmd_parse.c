...
void rmmi_custom_init_context(rmmi_custom_context_struct *rmmi_ptr_g)
{
    ...
    rmmi_ptr_g->rsp_mode.echo_flag = 0x00; // default echo off
#else 
...
#if defined (__SP_RIL_SUPPORT__) 
    rmmi_ptr_g->report_mode.einfo = 0x00000033; // ESMLA + ECFU + ESPEECH + STKPCI
#else
    ...
    rmmi_ptr_g->report_mode.ecellid = RMMI_ENABLE_REPORT;
#endif
