...
use strict;
my $logPath = "./build/$ARGV[0]/bin/log";
my $logPath_modis = "./build/$ARGV[0]/MoDIS/_BUILD_LOG";
...
{
  $logPath = "./build/$ARGV[0]/MoDIS/_BUILD_XGEN/bin/log";
...
my $errModule = 0;
while(defined($filename= <./build/$ARGV[0]/bin/log/*.log>)) {
...
if ($errModule == 0) {
  while(defined($filename= <./build/$ARGV[0]/bin/*.log>)) {
    open(F, $filename ) || die "I can't open $filename";
    my $errFlag = 0;
    while(<F>) {
