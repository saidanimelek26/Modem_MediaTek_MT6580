...
my ($CODEBASE_PATH,$THE_MF,$PROJECT_NAME,$FLAVOR,@CHANGE_SET) = @ARGV;
my @DEP_MODS = ();
...
open (MF,"<${THE_MF}") or die "can NOT open ${THE_MF}!!!\n";
while(<MF>)
{
	if (/^\s*(\w+)\s*=\s*(\w+)/)
	{
...
close (MF);
my @tmp = &NOTExistDEPMods("build/$PROJECT_NAME/$FLAVOR/bin/dep", 
                           &GetSRCMods("build/$PROJECT_NAME/$FLAVOR/bin/log/infomake.log"));
...
@DEP_MODS = &GetDEPMods("build/$PROJECT_NAME/$FLAVOR/bin/dep", \@CHANGE_SET);
if (@DEP_MODS)
{
...
Usage: $script CODEBASE THE_MF PROJECT_NAME FLAVOR <file1[ file2[ ...]] | \@files>

CODEBASE         Specify the code path
THE_MF           Specify project makefile, such as TIANYU28_DEMO.mak etc...
PROJECT_NAME     Specify project name, such as TIANYU28_DEMO etc...
FLAVOR           Specify project flavoe type, such as default, etc...
