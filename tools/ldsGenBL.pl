...
sub ldsGenBL_verno
{
    return "ldsGenBL m0.08 + ldsFrame.pm ".&ldsFrame::ldsFrame_verno;
...
sub GetBootingType
{
    ...
    $strBootingType = "NOR" if(&FileInfo::is_NOR() and !&FileInfo::is_SmartPhone());
...
sub GetEXT_BL_LOAD_MEM  #called by CollectMemorySetting()
{
    ...
    if(&FileInfo::is_NOR() and !FileInfo::is_SmartPhone())
    {
...
sub ParseVMAList # called by SetRegionList()
{
    ...
    foreach my $item (@$VMAList_aref)
    {
        ...
        my $bType = 1; 
        $bType = 0 if($strType =~ /^NOR$/i and !(&FileInfo::is_NOR() and !&FileInfo::is_SmartPhone()));
        $bType = 0 if($strType =~ /NAND/i and (&FileInfo::is_NOR() and !&FileInfo::is_SmartPhone()));
