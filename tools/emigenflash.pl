...
sub get_common_MDL_info
{
    ...
    for (1..$combo_mem_count)
    {
        ...
        if(($platform eq 'MT6255') || ($platform eq 'MT6280'))
        {
        	   if (($mem_dev_type eq 'LPDDR') or ($mem_dev_type eq 'DISCRETE_NAND') or ($mem_dev_type eq 'SERIALFLASH') or ($mem_dev_type eq 'LPDDR2'))
             {
        ...
        if (($platform ne 'MT6252') and ($platform ne 'MT6255'))  # MT6252 uses SIP
        {
        ...
        if ($platform ne 'MT6252')  # MT6252 uses SIP
        {
  ...
  if(($platform eq 'MT6255') || ($platform eq 'MT6280'))
  {
  	     if ( ($mem_dev_type eq 'LPDDR') or ($mem_dev_type eq 'DISCRETE_NAND') or ($mem_dev_type eq 'SERIAL_FLASH') or ($mem_dev_type eq 'LPDDR2'))
	      {
  ...
  else
  {
    ...
    if (($platform eq 'MT6255') || ($platform eq 'MT6280'))
    {
        	   ...
        	   ### Find the minimum RAM size in Mb
        	  if(($mem_dev_type eq 'LPDDR') || ($mem_dev_type eq 'LPDDR2'))
        	  {
        	  	for (1..$combo_mem_count)
        	  	{
        	  	my $cur_Size_Mb_ram;
...
sub flash_opt_gen_h_file_body
{
    ...
    ###
    my $smart_phone_define;
    if($MAKEFILE_OPTIONS_LOCAL->{'smart_phone_core'} ne "NONE")
    {
         $smart_phone_define = "#define __SMART_PHONE_PLATFORM__"	;
    }
    ###
    ...
    {
    	    if (($CUSTOM_MEM_DEV_OPTIONS_LOCAL->{MEMORY_DEVICE_TYPE} eq 'LPDDR') or ($CUSTOM_MEM_DEV_OPTIONS_LOCAL->{MEMORY_DEVICE_TYPE} eq 'DISCRETE_NAND') or ($CUSTOM_MEM_DEV_OPTIONS_LOCAL->{MEMORY_DEVICE_TYPE} eq 'LPDDR2'))
        {
...
sub combo_flash_id_h_file_body
{
    ...
    for (1..$CUSTOM_MEM_DEV_OPTIONS_LOCAL->{COMBO_MEM_ENTRY_COUNT})
    {
        ...
        ###get nand page size
        my $nand_page_size = $MDL_INFO_LIST_LOCAL->[$_]->{0}->{'NAND Page size(B)'};
        my $nand_block_size = $MDL_INFO_LIST_LOCAL->[$_]->{0}->{'NAND Block Size(KB)'};
        $nand_page_size = $nand_page_size . "," . $nand_block_size;
        print "page size:$nand_page_size\n";
        ###get flash id string
        ...
        {   // $MDL_INFO_LIST_LOCAL->[$_]->{0}->{'Part Number'}
            ...
            {$nand_page_size}, //page size, spare size, block size
...
__TEMPLATE__
    ...
    {
    	if(($MAKEFILE_OPTIONS_LOCAL->{'platform'} eq 'MT6255') and ($CUSTOM_MEM_DEV_OPTIONS_LOCAL->{MEMORY_DEVICE_TYPE} ne 'NOR_RAM_MCP'))
    	{
    		 $flash_id_str .= <<"__TEMPLATE__";
const kal_char FLASH_ID[] = "$MDL_INFO_LIST_LOCAL->[1]->{0}->{'Flash ID'}";
__TEMPLATE__
