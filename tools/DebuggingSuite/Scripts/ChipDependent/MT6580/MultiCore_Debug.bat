...
echo [MT6580 Multi-Core Debugging]
echo [A]ttach only
echo [R]eset and go
...
start %CVDPath%CVD.exe -Multi -Master -CORE=1 MultiCore/MultiCore_AP.csf

:: ping 3 s
ping 127.0.0.1 -n 3 -w 1000 > nul

echo [Step 1] Press any key to start MD1 ^(Cortex-R4^) debugger...
start %CVDPath%CVD.exe -Multi -Slave -CORE=2 MultiCore/MultiCore_MD.csf
...
echo Starting AP ^(Cortex-A7^) debugger...
start %CVDPath%CVD.exe -Multi -Master -CORE=1 MultiCore/MultiCore_AP_Attach.csf

:: ping 3 s
ping 127.0.0.1 -n 3 -w 1000 > nul

echo Start MD ^(Cortex-R4^) debugger...
start %CVDPath%CVD.exe -Multi -Slave -CORE=3 MultiCore/MultiCore_MD_Attach.csf
...
echo start MD ^(Cortex-R4^) debugger...
start %CVDPath%CVD.exe MultiCore/MultiCore_MD.cmm
