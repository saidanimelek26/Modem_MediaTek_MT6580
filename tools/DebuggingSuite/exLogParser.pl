...
#****************************************************************************
my ($boot_mode, $rtc, $execution_unit, $status, $stack_ptr, @stack_dump, $ext_queue_pending_cnt, 
...
# 3-3-1 >>> boot_mode (offset: +0x10, length: 1)
$boot_mode = $ex_log_raw[0x10];
$ex_log .= " - ex_type = $boot_mode_type[$boot_mode]\n";

# 3-3-2 >>> rtc (offset: +0x11, length: 8)
$rtc = "$ex_log_raw[0x17]/$ex_log_raw[0x15]/$ex_log_raw[0x14] $ex_log_raw[0x13]:$ex_log_raw[0x12]:$ex_log_raw[0x11]";
$ex_log .= " - rtc = $rtc\n";
