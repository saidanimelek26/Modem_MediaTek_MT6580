...
my ($FLASH_CFG,$file,$bin_name,$verno,$themf,$build_time_str,$tmpfolder,$scat,$sym_filename) = @ARGV;
...
#************************************************************
print "Input: $themf $scat $map_file $sym_file\n";
my $nSize = BasicMemInfo::GetTotalRAMUsageFromLDS($themf, $scat, $map_file, $sym_file, undef);
print "Memory size = $nSize\n";

my $secAdd = BasicMemInfo::GetPhysicalDummyEndOffset($themf, $scat, $map_file, $sym_file);
print "RFC Secure Address = $secAdd\n";

#MODEM mechanism

my $structure_size = 188;
my $check_header_str = "CHECK_HEADER";
my $header_version = "02";

#workaround for MT6589, since 89 is MP with old format, but also in WR8
if($platform eq "MT6589")
{
  $header_version = "01";
}
...
if($header_version eq "01")
{
  #2G/3G check
  #0x1: 2G
  #0x2: 3G
  #0x3: TDD_3G
  if($platform eq "MT6573" && $umts_mode_support eq "UMTS_TDD128_MODE_SUPPORT") {
  	print FILE pack("V1",3);
  }elsif(($l1_wcdma eq "TRUE") || ($l1_tdd128 eq "TRUE")) {
  	print FILE pack("V1",2);
  } else {
  	print FILE pack("V1",1);
  }
...
{ 
  ...
  #0x4: TG
  #0x5: 4G
  if($eutran_mode_support eq "EUTRAN_MODE") {
