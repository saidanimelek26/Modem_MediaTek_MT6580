...
typedef struct {
    ...
    /* forbidden plmn */
    kal_bool is_valid_ef_fplmn; /* Keep for backward compatible */
    ...
    kal_bool is_valid_ef_ehplmn;   //__R7_EHPLMN__ start
    kal_uint8 num_of_ehplmn; 
    kal_uint8 ef_ehplmn[12]; //__R7_EHPLMN__ end
    kal_bool is_valid_ef_ehplmnpi; // 23.122 R7 feature
...
typedef struct {
    ...
    kal_bool is_valid_ef_acsgl; /* __CSG_SUPPORT__ */	 
    kal_uint8 ef_acsgl_rec_size; /* record size of EF_ACSGL */	
    kal_uint8 *acsgl_data; /* Ctrl buffer with size MAX_SUPPORT_ACSGL_DATA_SIZE. SHALL be free by USIME */       	
...
typedef struct
{
    LOCAL_PARA_HDR
    kal_uint8    src_id;
    kal_uint8    sim_power_mode;
} sim_ap_simpower_req_struct;
