 ...
 *  If a trace is defined as °ßToday is %d-$d-%d°®, kal_trace simply sends the trace ID and the content of %d to PC. 
 *  That will reduce the communication traffic. In the chapter 2, we will introduce how to ass a new trace.
 *
 *
 * Property:
 *É‹	°ßMessage Compression°® type API
 *É‹	Cannot be invoked in LISR/HISR context.
 *É‹	Can send up to 128 bytes parameters
 *É‹	Suggested to use this API for most of the scenario.
 *
 * PARAMETERS
 ...
 *É‹	Cannot be invoked in LISR context.
 *É‹	Can send up to 128 bytes logs
 *É‹	This API can only be used in customer side to reduce the impact of ROM size increase.
 *É‹	Heavy resource consumption.
 *Éﬁ	  a.Caller°¶s RO-DATA
 *Éﬁ	 	b. Logging Bandwidth
 *
 * PARAMETERS
 *  mod_id: [IN] Module id
 ...
 ******************************************************************************/
  ...
  #define kal_bootup_print tst_sys_init_trace
    ...
    static __inline void kal_prompt_trace(module_type mod_id, const kal_char *fmt,...) {return ;}
    ...
    static __inline kal_int32 tst_trace_check_ps_filter_off(trace_class_enum trc_class, kal_uint32* module_id, kal_uint32 supportOption) {return 0;}
  #else
...
/* 
* Check if MDLogger is enable in SP logging
* (VM will change to another logging mode if it's disable)
*/
kal_bool tst_sp_md_logging_enable(void);
