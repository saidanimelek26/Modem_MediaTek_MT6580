...
/**
 * Description: Creates and intialises a DLIST_HEAD
 * structure before first use
 *
 * Notes:       Sets ulCount to zero and pFirst to
 * NULL.
 *
 * @return DLIST_HANDLE for the created doubly linked list,
 *         may be NULL if the required memory
 *         could not be allocated.
 */
extern DLIST_HANDLE DlistCreate(void);
extern DLIST_HANDLE DlistCreateWithMem(void *mem_ptr);
 ...
 */
extern kal_bool DlistDestroy(DLIST_HANDLE hList);
...
extern kal_bool DlistInsertAtStartWithMem(DLIST_HANDLE hList, void *pThis, void *mem_ptr);
