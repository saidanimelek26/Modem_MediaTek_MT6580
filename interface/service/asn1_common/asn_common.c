 ...
 * .
 * 
 * 07 19 2012 changshun.li
 * [MOLY00000935] [MOLY][MCDDLL] Decode primitive failure
...
U16 GetUperLengthDeterminant( AsnContext *pContext )
{
   ...
   {
      /* 7 bit length */
      length = getShortBits( pContext, 7 );
   }
   else
   if( !getShortBits( pContext, 1 ) )
   {
      /* 14 bit length */
      length = getShortBits( pContext, 14 );
   }
   else
   {
...
extern void putInt64( AsnContext *pContext, U32 numBits, U64 data )
{
   ...
   {
      /* Insert top 32 bits */
      putBits( pContext, 32, data >> (numBits - 32) );
      /* reduce amount to fit, and then mask off bits inserted */
      numBits -= 32;
      data &= lsbMask[numBits];
   }
   putBits( pContext, numBits, data );
}
