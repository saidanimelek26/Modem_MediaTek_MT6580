...
#define NVRAM_FS_MAKE_IMPORTNT_CS_PATH_FILENAME(buf, name) \
    kal_wsprintf( buf, "%s\\%s", NVRAM_FS_IMPORTNT_CS_DATAITEM_PATH, name);

#define NVRAM_FS_MAKE_IMPORTNT_CB_PATH_FILENAME(buf, name) \
    kal_wsprintf( buf, "%s\\%s", NVRAM_FS_IMPORTNT_CB_DATAITEM_PATH, name);
...
nvram_folder_enum nvram_query_folder_index_ex(nvram_category_enum category, kal_bool first_copy)
{
	...
	else if(NVRAM_IS_CATEGORY_IMPORTANT_C(category))
	{
    #if defined(__MTK_TARGET__) && defined(__NVRAM_IMPORTANT_C_PARTITIONS__)
        if (!first_copy)
            folder_index = NVRAM_NVD_IMPNTCB;
        else    
        folder_index = NVRAM_NVD_IMPNTCS;
	#else
	    folder_index = NVRAM_NVD_IMPNT;
	#endif	
    }
...
kal_char * nvram_query_work_path(nvram_folder_enum folder_idx)
{
    ...
    switch (folder_idx)
    {
        ...
        #if defined(__SMART_PHONE_MODEM__) || defined(__MODEM_CCCI_EXIST__)
        ...
        #endif
		#if defined(__MTK_TARGET__) && defined(__NVRAM_IMPORTANT_C_PARTITIONS__)
		case NVRAM_NVD_IMPNTCS:
		{
            return (kal_char *)NVRAM_FS_IMPORTNT_CS_DATAITEM_PATH;
        }
		case NVRAM_NVD_IMPNTCB:	
		{
            return (kal_char *)NVRAM_FS_IMPORTNT_CB_DATAITEM_PATH;
        }
		#endif
        #endif
        #ifdef __NVRAM_CUSTOM_SENSITIVE__
...
kal_wchar * nvram_query_file_name(nvram_folder_enum folder_idx, kal_char *nvramname, kal_wchar *filename)
{
    ...
    switch (folder_idx)
    {
		...
		case NVRAM_NVD_IMPNTCS:
		{
            NVRAM_FS_MAKE_IMPORTNT_CS_PATH_FILENAME(filename, nvramname);
            break;
        }
		case NVRAM_NVD_IMPNTCB:
		{
            NVRAM_FS_MAKE_IMPORTNT_CB_PATH_FILENAME(filename, nvramname);
            break;
        }					
        #endif
...
kal_wchar * nvram_query_ex_file_name(nvram_folder_enum folder_idx, kal_wchar *nvramname, kal_wchar *filename)
{
	...
	#ifdef __NVRAM_IMPORTANT_C_PARTITIONS__
	else if (folder_idx == NVRAM_NVD_IMPNTCS)
    {
        kal_wsprintf(filename, "%s\\%w", NVRAM_FS_IMPORTNT_CS_DATAITEM_PATH, nvramname);
    }
	else if (folder_idx == NVRAM_NVD_IMPNTCB)
    {
        kal_wsprintf(filename, "%s\\%w", NVRAM_FS_IMPORTNT_CB_DATAITEM_PATH, nvramname);
    }		
	#endif
    #endif
    #ifdef __NVRAM_CUSTOM_SENSITIVE__
