...
/* under construction !*/
        ...
        #endif
        #if defined(__MODEM_CCCI_EXIST__) && defined(__MODEM_CARD__)  // Hosted Dongle Data Card
...
#if !defined(__MODEM_CCCI_EXIST__) && !defined(__SMART_PHONE_MODEM__)
/*****************************************************************************
 * FUNCTION
...
static kal_bool nvram_version_conflict_resolve(kal_uint8 *sysrecord)
{
   ...
   nvram_trace(TRACE_FUNC, FUNC_NVRAM_VERSION_CONFLICT_RESOLVE, 4);
    ...
    /****** 3nd phase: reset NVRAM_CATEGORY_IMPORTANT_C ******/ 
	if (!nvram_reset_data_items(NVRAM_RESET_CUSTOM, NVRAM_APP_RESERVED, NULL, 0, 0))
	{
        return nvram_destruct_disk_table();
    }
