 ...
 ******************************************************/
        ...
        (status != NVRAM_IO_ERRNO_OK))
    {
        ...
        if (NVRAM_IS_CATEGORY_IMPORTANT_L4(ldi->category) == 0 &&
			NVRAM_IS_CATEGORY_IMPORTANT_C(ldi->category)== 0)   // Important partitions doesn't need auto recover
        {
...
//Read 2nd copy
    ...
    #if defined(__MTK_TARGET__) && defined(__SMART_PHONE_MODEM__)
		#if defined(__NVRAM_IMPORTANT_PARTITIONS__) || defined(__NVRAM_IMPORTANT_C_PARTITIONS__)
		{
            ...
            kal_uint16 rec_amount)
{
    ...
    #endif 
		case NVRAM_RESET_CUSTOM:
		{
			nvram_reset_category(NVRAM_CATEGORY_IMPORTANT_C, 0);
			break;
		}
