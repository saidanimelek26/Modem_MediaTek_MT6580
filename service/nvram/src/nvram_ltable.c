...
static void nvram_assign_table_entry(nvram_lid_enum position, nvram_ltable_entry_struct* ldi)
{
    ...
    if (NVRAM_IS_CATEGORY_CALIBRAT(ldi->category))
    {
        ...
        ldi->attr &= ~NVRAM_ATTR_BACKUP_FAT;
    #else
        #if !defined(__LOW_COST_SUPPORT_COMMON__) && !defined(__SMART_PHONE_MODEM__) && !defined(__MODEM_CCCI_EXIST__)
        ldi->attr |= NVRAM_ATTR_MULTIPLE;
        #endif
        ldi->attr &= ~NVRAM_ATTR_BACKUP_RAW;
        ldi->attr &= ~NVRAM_ATTR_BACKUP_FAT;
 ...
 */
    ...
    if (NVRAM_IS_CATEGORY_IMPORTANT(ldi->category) 
		...
		|| NVRAM_IS_CATEGORY_IMPORTANT_C(ldi->category))
    {
        NVRAM_TURNOFF_ATTR_FACTOY_RESET(ldi->attr);
    ...
    //      Remove multiple and msp attribute
    //***************************************************************************
#ifdef __NVRAM_PSEUDO_MERGE__
    if (NVRAM_IS_ATTR_PACKAGE(ldi->attr))
    {
        /*
         * When pesudo merge is turn on, nvram will use ldi->description as file offset
         *
         */
    ...
    //***************************************************************************
#if defined(__NVRAM_COMPRESS_SUPPORT__)
        ...
        !NVRAM_IS_CATEGORY_SECUPACK(ldi->category))
    {
        if (NVRAM_IS_CATEGORY_CUSTPACK(ldi->category))
        {
             ldi->attr |= NVRAM_ATTR_ZIP_DEFAULT;
        }
        else if (ldi->LID >= NVRAM_EF_LAST_LID_CORE
            && !NVRAM_IS_ATTR_NOT_ZIP_DEFAULT(ldi->attr)
