...
#ifdef __BROM_USBDL_V2__
void addWDTRstCnt() {
	*RETN_DAT3 = *RETN_DAT3 + 0x100;		// The last byte is for RTEN_FLAG		
}

void resetWDTRstCnt() {
	*RETN_DAT3 = (*RETN_DAT3) & 0xff0000ff;
}

kal_uint32 getWDTRstCnt() {
	return ( *(RETN_DAT3) & 0x00ffff00 ) >> 8;
}

void copyRGUtoData3() {
	*RETN_DAT3 = 0x4e000000 | *RETN_FLAG;
}
...
/* under construction !*/
         ...
         {
         	   /* DL flag is now cleared by MAUI, since we don't know if the last download success! */
            //ClearUSBDLFlag();
            BL_PRINT(LOG_INFO, "DL flag will be cleared by MAUI side...\r\n");
         }
      }
      else
      {
...
void WatchDogResetCheck(void) {
	if( IsWatchDogRestart() == KAL_TRUE ) {
		addWDTRstCnt();
		dbg_print( "WDT reset detected, count: %d \r\n", getWDTRstCnt() );
		
		if( getWDTRstCnt() > WDT_RST_THRESHOLD ) {
			dbg_print( "WDT reset could excced the threshold(%d), open com port...\r\n", WDT_RST_THRESHOLD );
			resetWDTRstCnt();
			WacthDogEnable();
      SetRetnTimeout(BROM_USBDL_V2_TIMEOUT);
    	SetRetnFlag(RETN_USBDL_BY_FLAG_EN);      
     	ClrRetnFlag(RETN_USBDL_TYPE_MASK);
			SetWDTNormalResetFlag();
      WatchDogResetAll();
