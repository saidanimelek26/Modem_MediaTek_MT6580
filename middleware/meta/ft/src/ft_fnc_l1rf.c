...
void FT_FtRfTestReq(ft_rf_test_req_T *req, peer_buff_struct *p_peer_buff_ptr)
{
    ...
    switch (req->type)
    {
        ...
        case RF_TEST_CMD_GET_IMMEDIATE_BSI:
        {
            ...
            ptrMsg->param.GetBSI.ok = KAL_TRUE;
            /* FT_SEND_MSG(src_mod, dest_mod, sap_id, msg_id, ilm_ptr) */
            FT_SEND_MSG(MOD_FT, MOD_TST, FT_TST_SAP, MSG_ID_FT_TO_TST, &ilm_ptr);
            break;
        }
...
#if MD_DRV_IS_NSFT_LIST_MODE_SUPPORT == 1
        case RF_TEST_CMD_LIST_MODE_NSFT_START:
        {
            kal_uint8              i, j;
            kal_uint16             pdu_length;
            nsft_lm_req_struct     nsft_lm_req;
            NsftListCmd_t*         lm_cmd_ptr;
			RfTestCmdListModeNSFT* peer_ptr;
            RfTestCmdListModeNSFTCommonParam* ptr;
            // store rf token id, because this command won't return immediately.
            ft_gl_rf_token = req->header.token;

            // get the parameter from peer buffer
            peer_ptr = (RfTestCmdListModeNSFT*)get_peer_buff_pdu( p_peer_buff_ptr, &pdu_length );

            for(i = 0; i < req->param.List_Mode_NSFT_start.cmd_count; i++)
            {
                ptr = (RfTestCmdListModeNSFTCommonParam*)&peer_ptr->param[i];
                switch(ptr->cmd_type)
                {
                    case NSFT_LM_CMD_SYNC:
                    {
                        memset(&nsft_lm_req, 0, sizeof(nsft_lm_req_struct));
                        
                        nsft_lm_req.commandType = NSFT_LM_CMD_SYNC;
                        lm_cmd_ptr = (NsftListCmd_t*)&nsft_lm_req.commandBuffer;
                        lm_cmd_ptr->sync.command_id       = i;
                        lm_cmd_ptr->sync.RF_band          = peer_ptr->param[i].sync.band;
                        lm_cmd_ptr->sync.ARFCN            = peer_ptr->param[i].sync.arfcn;
                        lm_cmd_ptr->sync.sync_power       = peer_ptr->param[i].sync.sync_power;
                        lm_cmd_ptr->sync.sync_slot_offset = peer_ptr->param[i].sync.sync_slot_offset;
                        L1TST_NsftList_message_handler(&nsft_lm_req);
#if defined(__MTK_INTERNAL__) && defined(_DEBUG)
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
                    ...
                    case NSFT_LM_CMD_TRIGGER:
                    {                        
                        memset(&nsft_lm_req, 0, sizeof(nsft_lm_req_struct));

                        nsft_lm_req.commandType = NSFT_LM_CMD_TRIGGER;
                        lm_cmd_ptr = (NsftListCmd_t*)&nsft_lm_req.commandBuffer;
                        lm_cmd_ptr->trigger.command_id = i;
                        lm_cmd_ptr->trigger.RF_band    = peer_ptr->param[i].trigger.band;
                        lm_cmd_ptr->trigger.ARFCN      = peer_ptr->param[i].trigger.arfcn;
                        lm_cmd_ptr->trigger.TSC        = peer_ptr->param[i].trigger.tsc;
                        lm_cmd_ptr->trigger.RTX_types  = peer_ptr->param[i].trigger.trx_type;
                        lm_cmd_ptr->trigger.TX_power   = peer_ptr->param[i].trigger.tx_power;
                        L1TST_NsftList_message_handler(&nsft_lm_req);
#if defined(__MTK_INTERNAL__) && defined(_DEBUG)
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
                    ...
                    case NSFT_LM_CMD_CHMEAS:
                    {
                        memset(&nsft_lm_req, 0, sizeof(nsft_lm_req_struct));

                        nsft_lm_req.commandType = NSFT_LM_CMD_CHMEAS;
                        lm_cmd_ptr = (NsftListCmd_t*)&nsft_lm_req.commandBuffer;
                        lm_cmd_ptr->test.command_id = i;
                        lm_cmd_ptr->test.RF_band    = peer_ptr->param[i].test.band;
                        lm_cmd_ptr->test.ARFCN      = peer_ptr->param[i].test.arfcn;
                        lm_cmd_ptr->test.TSC        = peer_ptr->param[i].test.tsc;
                        lm_cmd_ptr->test.RTX_types  = peer_ptr->param[i].test.trx_type;
                        lm_cmd_ptr->test.RX_power   = peer_ptr->param[i].test.rx_power;
                        for(j = 0; j < 8 ; j++)
                        {
                           lm_cmd_ptr->test.TX_power[j]     = peer_ptr->param[i].test.tx_pcl[j];                 
                           lm_cmd_ptr->test.repeat_count[j] = peer_ptr->param[i].test.repeat_count[j];
                        }    
                        L1TST_NsftList_message_handler(&nsft_lm_req);
#if defined(__MTK_INTERNAL__) && defined(_DEBUG)
/* under construction !*/
...
/* under construction !*/
            ...
            memset(&nsft_lm_req, 0, sizeof(nsft_lm_req_struct));           
            nsft_lm_req.commandType = NSFT_LM_CMD_STOP;
     		L1TST_NsftList_message_handler(&nsft_lm_req);

            // did not return the CNF to PC side,
            // because we wait for L1 call FT_DispatchReports with type: ReportListModeNSFT
	        break;
        }
#endif // #if MD_DRV_IS_NSFT_LIST_MODE_SUPPORT == 1
...
kal_uint32 FT_L1RfCheckFunctionSuppported(kal_uint32 query_op_code)
{
    ...
    if( RF_TEST_CMD_END > query_op_code )
    {
        switch(query_op_code)
        {
            ...
            case RF_TEST_CMD_NSFT_CHANGE_POWER:
#if MD_DRV_IS_NSFT_LIST_MODE_SUPPORT == 1
            case RF_TEST_CMD_LIST_MODE_NSFT_START:      /** 2G list mode nsft*/
#endif // #if MD_DRV_IS_NSFT_LIST_MODE_SUPPORT == 1
            {
                cnf_code = FT_CNF_OK;
                break;
...
#if MD_DRV_IS_NSFT_LIST_MODE_SUPPORT == 1 
void FT_ftRfListModeNsftCnf(ilm_struct *ilm_ptr)
{
    switch(ilm_ptr->msg_id)
    {
        case MSG_ID_FT_TO_TST:
	    {
            ilm_struct	cnf_msg;			
            ft_rf_test_cnf_T* ptrMsg;
	        RfTestResultListModeNSFTParam *pdu_ptr;
            NsftList_RPT_Q_ENTRY_T* L1ListModeReport;
			void *lp_ptr;
            //NsftListRpt_Com_t* common;
	        peer_buff_struct *peer_buff_ret;
            kal_uint16 pdu_length, pdu_length2;
            kal_uint32 report_count;
            int i; 
            // initialize
            pdu_ptr = NULL;
    	    peer_buff_ret = NULL;
	        ...
	        FT_ALLOC_MSG(&cnf_msg, sizeof(ft_rf_test_cnf_T));
            ptrMsg = (ft_rf_test_cnf_T*)cnf_msg.local_para_ptr;
            /* if ptrMsg != NULL*/
            ptrMsg->header.ft_msg_id = FT_RF_TEST_CNF_ID;
            ptrMsg->header.token = ft_gl_rf_token;
            ptrMsg->type = RF_TEST_CMD_LIST_MODE_NSFT_START;

            lp_ptr = (void *)((kal_uint8 *)(ilm_ptr->local_para_ptr) + sizeof(local_para_struct));
            kal_mem_cpy(&report_count, lp_ptr, sizeof(kal_uint32));
            ptrMsg->param.List_Mode_NSFT_result.report_count = report_count;            

            if( NULL != (peer_buff_ret=construct_peer_buff(sizeof(RfTestResultListModeNSFTParam), 0, 0, TD_RESET)) )
            {
                pdu_ptr = (RfTestResultListModeNSFTParam*)get_peer_buff_pdu( peer_buff_ret, &pdu_length );
                peer_buff_ret->pdu_len = sizeof(RfTestResultListModeNSFTParam);
                memset(pdu_ptr, 0, sizeof(RfTestResultListModeNSFTParam));

                L1ListModeReport = (NsftList_RPT_Q_ENTRY_T*)get_peer_buff_pdu( ilm_ptr->peer_buff_ptr, &pdu_length2 );
 
                for(i = 0; i < report_count; i++)
                {
                    switch(L1ListModeReport->outputParameter.common.type)
                    {
                        case NSFT_LM_CMD_SYNC:
                        {
                            pdu_ptr->cmd_type[i]                  = L1ListModeReport->outputParameter.sync.common.type;
                            pdu_ptr->report[i].sync.common.status = L1ListModeReport->outputParameter.sync.common.status;
                            pdu_ptr->report[i].sync.detected_foe  = L1ListModeReport->outputParameter.sync.detected_foe;
                            pdu_ptr->report[i].sync.new_afc_dac   = L1ListModeReport->outputParameter.sync.new_afc_dac;
                            pdu_ptr->report[i].sync.retry_count   = L1ListModeReport->outputParameter.sync.retry_counts;   
                            break;
                        }                        
                        case NSFT_LM_CMD_TRIGGER:
                        {
                            pdu_ptr->cmd_type[i]                     = L1ListModeReport->outputParameter.trigger.common.type;
                            pdu_ptr->report[i].trigger.common.status = L1ListModeReport->outputParameter.trigger.common.status;
                            break;
                        }
                        case NSFT_LM_CMD_CHMEAS:
                        {
                            pdu_ptr->cmd_type[i]                   = L1ListModeReport->outputParameter.test.common.type;
                            pdu_ptr->report[i].test.common.status  = L1ListModeReport->outputParameter.test.common.status;
                            pdu_ptr->report[i].test.BER_frame_count= L1ListModeReport->outputParameter.test.BER_frame_count;
                            pdu_ptr->report[i].test.BER_sum        = L1ListModeReport->outputParameter.test.BER_sum;
                            pdu_ptr->report[i].test.RX_level       = L1ListModeReport->outputParameter.test.RX_level;
                            break;
                        }
                        case NSFT_LM_CMD_STOP:
                        default:
                        {                            
                            break;
                        }
		        ...
		        if( NULL != peer_buff_ret ) 
		        {
                    cnf_msg.peer_buff_ptr = peer_buff_ret;
                }
            }
            FT_SEND_MSG(MOD_FT, MOD_TST, FT_TST_SAP, MSG_ID_FT_TO_TST, &cnf_msg);
			break;
		}
    }
}
