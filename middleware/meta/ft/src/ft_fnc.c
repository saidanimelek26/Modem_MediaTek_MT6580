...
extern kal_char* release_bb_chip();
extern kal_char* release_dsp_fw();
extern kal_char* release_dsp_ptch();
extern kal_char* release_verno();
extern kal_char* release_hw_ver();
/*******************************************************************************
 *
 *  Utility Functions
 *
...
void FT_GetVersionInfo(void) {
    ...
    strcpy((kal_char *)ptrMsg->bb_chip, release_bb_chip());
    // assign ECO version
    kal_mem_set(ptrMsg->eco_ver, 0, 4);
    // assign DSP firmware version string
    strcpy((kal_char *)ptrMsg->dsp_fw, release_dsp_fw());
    // assign DSP patch version string
    strcpy((kal_char *)ptrMsg->dsp_patch, release_dsp_ptch());
    // assign S/W version string
    strcpy((kal_char *)ptrMsg->sw_ver, release_verno());
    // assign H/W version string
    strcpy((kal_char *)ptrMsg->hw_ver, release_hw_ver());
...
void ft_send_in_proc_call_req(ft_in_proc_call_type func, kal_uint32 func_arg1, void *func_arg2)
{
    ...
    ptrMsg = (ft_in_proc_call_req_struct *)construct_local_para(sizeof(ft_in_proc_call_req_struct), TD_CTRL);
    if(NULL == ptrMsg)
    {
        ASSERT(0);    
    }    
