...
void FT_UL1TST_SEND_CNF_BACK(ilm_struct *ptrMsg_ul1tst)
{
    ...
    // just do nothing, if the confirm type dees not belong to the following types
    // do this check will prevent us from allocating message without sending it.
    if( ptrMsgbody->type != UL1TST_CMD_RSSI      &&
        ptrMsgbody->type != UL1TST_CMD_RSSI_CONT &&
        ptrMsgbody->type != UL1TST_CMD_RSCP      &&
        ptrMsgbody->type != UL1TST_CMD_AFC       &&
#if defined (__UL1_HS_PLATFORM__) || defined (__UL1_HS_PLUS_PLATFORM__)
        ptrMsgbody->type != UL1TST_CMD_AFC_EXT   &&
#endif // #if defined (__UL1_HS_PLATFORM__) || defined (__UL1_HS_PLUS_PLATFORM__)
        ptrMsgbody->type != UL1TST_CMD_MQC       &&
        ptrMsgbody->type != UL1TST_CMD_NSFT      &&
        ptrMsgbody->type != UL1TST_CMD_RSSI_RXD  &&  /**< \breif R8 get both rx path RSSI */
        ptrMsgbody->type != UL1TST_CMD_ANT_MODE   &&   /**< \breif R8 set rx path RSSI */
/*R8*/  ptrMsgbody->type != UL1TST_CMD_PA_DRIFT_SWITCH  &&                      
/*R8*/  ptrMsgbody->type != UL1TST_CMD_RX_CELL_POWER                      
#if defined (__UL1_HS_PLATFORM__) || defined (__UL1_HS_PLUS_PLATFORM__)
        && ptrMsgbody->type != UL1TST_CMD_MQC_OCTLEV 
        && ptrMsgbody->type != UL1TST_CMD_MQC_EXT 
#endif // #if defined (__UL1_HS_PLATFORM__) || defined (__UL1_HS_PLUS_PLATFORM__)
#if defined (__HSDPA_SUPPORT__)
        &&
        ptrMsgbody->type != UL1TST_CMD_HSDPA_NSFT
#endif // #if defined (__HSDPA_SUPPORT__)
#if defined (__HSUPA_SUPPORT__)
        &&
        ptrMsgbody->type != UL1TST_CMD_HSUPA_NSFT
#endif // #if defined (__HSUPA_SUPPORT__)
      )
    {
        return;
    }
    if( ptrMsgbody->type != UL1TST_CMD_MQC
#if defined (__UL1_HS_PLATFORM__) || defined (__UL1_HS_PLUS_PLATFORM__)
            &&
        ptrMsgbody->type != UL1TST_CMD_MQC_OCTLEV
#endif // #if defined (__UL1_HS_PLATFORM__) || defined (__UL1_HS_PLUS_PLATFORM__)
