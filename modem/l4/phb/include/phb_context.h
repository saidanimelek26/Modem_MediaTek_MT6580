...
typedef struct
{
    ...
    kal_bool            phb_seek_done;
    kal_uint8           phb_seek_adn_bitmap[(ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8];
    kal_uint8           phb_seek_iap_bitmap[(ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8];
#ifdef __PHB_USIM_SYNC__
    kal_uint8           phb_seek_pbc_bitmap[(ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8];
#endif
