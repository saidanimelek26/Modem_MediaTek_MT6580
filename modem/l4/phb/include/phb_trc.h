...
BEGIN_TRACE_MAP(MOD_PHB)
   ...
   TRC_MSG(INFO_PHB_WRONG_RES_IF_CB, "[PHB] Wrong RES if CB created ID = %d, res_msg_id = %d, IO_cnf_receive = %x")
   TRC_MSG(INFO_PHB_WRONG_RES_IF_NOT_CB, "[PHB] Wrong RES if CB not created access_id = %d, ilm_msg_id = %d")
   TRC_MSG(INFO_PHB_NEW_CB_OCCUPIED, "[PHB] new control block occupied value = %d, CB ID = %x")
