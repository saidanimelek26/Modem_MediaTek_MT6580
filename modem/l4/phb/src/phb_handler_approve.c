...
#include "phb_sap.h"
#include "sim_common_enums.h" // for check_sim_card_status()
...
#ifndef __PHB_IGNORE_PAUSE_FOR_FDN_APPROVAL__
#define __PHB_IGNORE_PAUSE_FOR_FDN_APPROVAL__
#endif
...
void phb_approve_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    if (ilm_ptr != NULL)
    {
        ...
        if (l4cphb_approve_req->approval_type == APPROVAL_TYPE_NONE)
        {
            /* l4c owner wants here to go nothing and return SUCCESS, if approve_type = APPROVAL_TYPE_NONE) */
            phb_approve_confirm(
                PHB_ERRNO_SUCCESS,
                PHB_NONE,
        ...
        if (phb_ptr->dn_type == TYPE_NONE)
        {
            ...
            if ((l4cphb_approve_req->approval_type == APPROVAL_TYPE_ECC_ONLY || l4cphb_approve_req->approval_type == APPROVAL_TYPE_FULL)
                && phb_approve_ecc(NULL, tel_number, &phb_entry, &esc) == KAL_TRUE)
            {
            /**
             ...
             */
                ...
                phb_approve_confirm(PHB_ERRNO_SUCCESS, PHB_ECC, l4cphb_approve_req->src_id, NULL, &phb_entry, esc);
                return;
            }
            else
            {
       ...
       */
        if ((phb_ptr->state == PHB_STATE_READY) && (control_block != NULL))
        {
            ...
            if (control_block->proc_stage == APPROVE_NONE)
            {
                ...
                /* Approve ECC! */
                if ((l4cphb_approve_req->approval_type == APPROVAL_TYPE_ECC_ONLY || l4cphb_approve_req->approval_type == APPROVAL_TYPE_FULL)
                    && phb_approve_ecc(control_block, tel_number, &control_block->temp_entry, &esc) == KAL_TRUE)
                {
                ...
                {
                    if ((tel_number->addr_length == 3 && tel_number->addr_bcd[1] == 0x00 && tel_number->addr_bcd[2] == 0xF0) ||
                        (tel_number->addr_length == 2 && tel_number->addr_bcd[1] == 0x80) ||
                        (tel_number->addr_length == 3 && tel_number->addr_bcd[1] == 0x11 && tel_number->addr_bcd[2] == 0xF0) ||
                        (tel_number->addr_length == 3 && tel_number->addr_bcd[1] == 0x21 && tel_number->addr_bcd[2] == 0xF0) ||
                        (tel_number->addr_length == 3 && tel_number->addr_bcd[1] == 0x11 && tel_number->addr_bcd[2] == 0xF8) ||
                        (tel_number->addr_length == 3 && tel_number->addr_bcd[1] == 0x11 && tel_number->addr_bcd[2] == 0xF9) ||
                        (tel_number->addr_length == 3 && tel_number->addr_bcd[1] == 0x99 && tel_number->addr_bcd[2] == 0xF9))
                    {
             ...
             */
                ...
                if ((l4cphb_approve_req->approval_type == APPROVAL_TYPE_FDN_ONLY || l4cphb_approve_req->approval_type == APPROVAL_TYPE_FULL)
        ...
        else
        {
            ...
            if (control_block != NULL)
            {
                ...
                if ((phb_ptr->state == PHB_STATE_NOT_READY_FDN_READY) &&
                    (l4cphb_approve_req->approval_type == APPROVAL_TYPE_FDN_ONLY || l4cphb_approve_req->approval_type == APPROVAL_TYPE_FULL) &&
                    (phb_approve_fdn(control_block, tel_number, l4cphb_approve_req) == KAL_TRUE))
                {
                    return;
                }
    ...
    else
    {
        ...
        if (control_block->proc_stage == APPROVE_FDN || control_block->proc_stage == APPROVE_FDN_ONE_CHARACTER)
        {
             ...
             */
                ...
                phb_approve_confirm(PHB_ERRNO_FAIL, control_block->type, control_block->src_id, control_block, NULL, 0);
                return;
            }
          ...
          */
            control_block->proc_stage = APPROVE_FDN;
            phb_approve_confirm(PHB_ERRNO_SUCCESS, control_block->type, control_block->src_id, control_block, NULL, 0);
            return;
        }
        /* error */
        else
                    ...
                    kal_uint8 *esc)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    call_type = ECC_SIM1_CALL + phb_which_sim_ex();

    result = ecc_custom_verify_emergency_number(
        tel_number->addr_bcd,
                    ...
                    l4cphb_approve_req_struct *l4cphb_approve_req)
{
    ...
    if ((phb_ptr->dn_type == TYPE_FDN) || (phb_ptr->dn_type == TYPE_FDN_BDN))
    {
        ...
        control_block->type = PHB_FDN;
        control_block->candidate = (kal_uint16) PHB_INVALID_VALUE;

        control_block->candidate_name_num_index = 
            phb_se_compare_tel_number_with_first_character(control_block->type, tel_number, 0, &control_block->candidate, &is_more_candidates);
        if ((control_block->candidate != (kal_uint16) PHB_INVALID_VALUE) &&
            (control_block->candidate_name_num_index != NULL))
        {
           /**
        ...
        {
            control_block->candidate_name_num_index = phb_se_search_by_tel_num_one_by_one(control_block->type, tel_number, 0, &control_block->candidate, &is_more_candidates);
...
static void phb_approve_rest_for_PHB_not_ready(l4cphb_approve_req_struct *l4cphb_approve_req)
{
    ...
    */
    ...
    {
        phb_approve_confirm(PHB_ERRNO_FAIL, PHB_NONE, l4cphb_approve_req->src_id, NULL, NULL, 0);
        return;
    }
    else if ((dn_type == TYPE_ADN) || (dn_type == TYPE_BDN) || (dn_type == TYPE_ADN_BDN) ||
            /**
             * What the hell is this type!?
             * Currently, treat it as FDN disabled.
             * Hence, it is ALWAYS approved, but searching is unnecessary,
             ...
             */
             (dn_type == TYPE_NONE))
    {
        phb_approve_confirm(PHB_ERRNO_SUCCESS, PHB_NONE, l4cphb_approve_req->src_id, NULL, NULL, 0);
        return;
    }
...
static void phb_approve_continue(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
#ifdef __PHB_IGNORE_PAUSE_FOR_FDN_APPROVAL__
	kal_uint8 i;
#endif
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    ...
    else
    {
...
#ifdef __PHB_IGNORE_PAUSE_FOR_FDN_APPROVAL__
		for(i = L4_ADDR_BCD_TEL_NUM_OFFSET ; i <= candidate_pattern->addr_length ; i++)
		{
			if ((candidate_pattern->addr_bcd[i] & 0x0f) == 0x0c)
			{
				candidate_pattern->addr_bcd[i] = candidate_pattern->addr_bcd[i] | 0x0f;
				break;
			}
			else if ((candidate_pattern->addr_bcd[i] & 0xf0) == 0xc0)
			{
				candidate_pattern->addr_bcd[i] = candidate_pattern->addr_bcd[i] | 0xf0;
				break;
			}	
		}
#endif
        ...
        if (control_block->proc_stage == APPROVE_FDN_ONE_CHARACTER)
        {
            ...
            else
            {
                ...
                name_num_index_type *name_num_index;
                data_entry_struct *data_entry;
                kal_uint8 fdn_proc_stage;

                /* set storage, record_index, primary_ID, and secondary_ID */
                name_num_index = (name_num_index_type*) control_block->candidate_name_num_index;
                control_block->candidate_name_num_index = 
                    phb_se_compare_tel_number_with_first_character(control_block->type, given, (control_block->candidate + 1), &control_block->candidate, &is_more_candidates);
                if ((control_block->candidate != (kal_uint16) PHB_INVALID_VALUE) &&
                    (control_block->candidate_name_num_index != NULL))
                {
                   /**
                ...
                {
                    control_block->candidate_name_num_index = phb_se_search_by_tel_num_one_by_one(control_block->type, given, 0, &control_block->candidate, &is_more_candidates);
                    fdn_proc_stage = APPROVE_FDN;
        ...
        else if (control_block->proc_stage == APPROVE_FDN)
        {
            ...
            else
            {
                ...
                /* set storage, record_index, primary_ID, and secondary_ID */
                name_num_index = (name_num_index_type*) control_block->candidate_name_num_index;
                control_block->candidate_name_num_index = phb_se_search_by_tel_num_one_by_one(control_block->type, given, (control_block->candidate + 1), &control_block->candidate, &is_more_candidates);
                if ((control_block->candidate == (kal_uint16) PHB_INVALID_VALUE) ||
                    (control_block->candidate_name_num_index == NULL))
                {
                    control_block->proc_stage = APPROVE_FDN;
...
static kal_bool phb_compare_tel_number_prefix(l4_addr_bcd_struct *given, l4_addr_bcd_struct *candidate)
{
    ...
    */
    ...
    if (compare_n_bcd_for_wild_char(
            &candidate->addr_bcd[L4_ADDR_BCD_TEL_NUM_OFFSET],
...
/*****************************************************************************
 * FUNCTION
 *  phb_update_nw_ecc_list_handler
 * DESCRIPTION
 *  update the network ecc list
 *  
 *  Algorightm:
 * PARAMETERS
 *  
 ...
 *  
 *****************************************************************************/
void phb_update_nw_ecc_list_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_uint8 i, ecc_src, total_ecc_num;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_trace(TRACE_FUNC, FUNC_PHB_UPDATE_NW_ECC_LIST);

    if (ilm_ptr != NULL)
    {
        l4cphb_update_nw_ecc_req_struct *l4cphb_update_nw_ecc_req;

        l4cphb_update_nw_ecc_req = (l4cphb_update_nw_ecc_req_struct*) ilm_ptr->local_para_ptr;
        
        ecc_src = ECC_SRC_NW1 + phb_which_sim_ex();

        total_ecc_num = l4cphb_update_nw_ecc_req->no_ecc;
        /* update ecc to applib */
        for (i = 0; i < total_ecc_num; i++)
        {
            ecc_custom_update_emergency_number(
                l4cphb_update_nw_ecc_req->ecc_list[i].tel_number.addr_bcd,
                ECC_ENCODING_BCD,
                ecc_src,
                l4cphb_update_nw_ecc_req->ecc_list[i].category,
                NULL,
                i + 1,
                total_ecc_num);
        }
