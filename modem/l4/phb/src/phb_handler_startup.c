...
static void phb_search_adn_record_cnf(ilm_struct *ilm_ptr, control_block_type *cblk);
static kal_bool phb_search_adn_record_req(control_block_type *cblk);
...
#ifdef __SIM_SEARCH_RECORD_SUPPORT__
static void phb_search_iap_record_cnf(ilm_struct *ilm_ptr, control_block_type *cblk);
static kal_bool phb_search_iap_record_req(control_block_type *cblk);
#endif /* __SIM_SEARCH_RECORD_SUPPORT__ */
...
#ifdef __PHB_USIM_SYNC__
#ifdef __SIM_SEARCH_RECORD_SUPPORT__
static void phb_search_pbc_record_cnf(ilm_struct *ilm_ptr, control_block_type *cblk);
static kal_bool phb_search_pbc_record_req(control_block_type *cblk);
#endif /* __SIM_SEARCH_RECORD_SUPPORT__ */
#endif
...
static phb_query_entry_struct phb_sim_info_tbl[] =
{
...
#ifdef __SIM_SEARCH_RECORD_SUPPORT__
    {
        ...
        SERVICE_ADN,
        SIM_REC_NUM,   
        phb_search_adn_record_req, 
        phb_search_adn_record_cnf, 
        SIMU_DESC(FILE_G_ADN_IDX)
    },
#endif /* __SIM_SEARCH_RECORD_SUPPORT__ */
#endif        
...
#ifdef __SIM_SEARCH_RECORD_SUPPORT__
    {
        ...
        SERVICE_END,
        SIM_REC_NUM,   
        phb_search_iap_record_req, 
        phb_search_iap_record_cnf, 
        SIMU_DESC(FILE_G_IAP_IDX)
    },
#endif /* __SIM_SEARCH_RECORD_SUPPORT__ */
#ifndef __PHB_IAP_CACHE_REMOVAL__
...
#ifdef __SIM_SEARCH_RECORD_SUPPORT__
    {
        ...
        SERVICE_END,
        SIM_REC_SIZE,   
        phb_search_pbc_record_req, 
        phb_search_pbc_record_cnf, 
        SIMU_DESC(FILE_G_PBC_IDX)
    },
#endif /* __SIM_SEARCH_RECORD_SUPPORT__ */
    {
...
static void phb_read_iap_cnf(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (cblk->record_index < ADN_INDEX_MAX_ENTRIES_COUNT)
    {
        kal_mem_cpy(phb_ptr->iap_array[cblk->record_index - 1], cblk->data, PHB_MAX_TYPE2_ENTRIES);
    }
        ...
        cblk->actual_count < ADN_INDEX_MAX_ENTRIES_COUNT)
    {
        ++cblk->record_index;
    #ifdef __SIM_SEARCH_RECORD_SUPPORT__
        if (phb_ptr->phb_seek_done)
        {
            kal_uint16 seek_byte = (cblk->record_index - 1) / 8;
            kal_uint16 seek_bit = (cblk->record_index - 1) % 8;
            while ((seek_byte < (ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8) && (phb_ptr->phb_seek_iap_bitmap[seek_byte] & (1 << seek_bit)) == 0)
            {
                ++cblk->record_index;
                seek_byte = (cblk->record_index - 1) / 8;
                seek_bit = (cblk->record_index - 1) % 8;
            }
        }
    #endif /* __SIM_SEARCH_RECORD_SUPPORT__ */
        if (cblk->record_index > phb_ptr->data_desc[DATA_DESC_ADN].record_num)
        {
            (*cblk->controller) (NULL, cblk);
            return;
        }
    #ifdef __SIM_SEARCH_RECORD_SUPPORT__
        kal_trace(TRACE_INFO, FUNC_PHB_SIM_SEARCH_RECORD_READ_IAP, phb_ptr->phb_seek_done, cblk->record_index);
    #endif
        phb_issue_IO_read(cblk);
    }
    else
...
static kal_bool phb_read_iap_req(control_block_type *cblk)
{
    ...
    if (phb_ptr->data_desc[DATA_DESC_IAP].record_num > 0)
    {
        ...
        cblk->storage = phb_which_sim_storage();
        memset(phb_ptr->iap_array, 0x00, ADN_INDEX_MAX_ENTRIES_COUNT * PHB_MAX_TYPE2_ENTRIES);
...
static void phb_search_iap_record_cnf(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    sim_search_record_cnf_struct *sim_iap_record_cnf;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    sim_iap_record_cnf = (sim_search_record_cnf_struct*) ilm_ptr->local_para_ptr;

    if (cblk->actual_count <= phb_ptr->data_desc[DATA_DESC_PBR].record_num)
    {
        kal_uint8 i;
        kal_uint16 record_index, seek_byte, seek_bit;
        kal_uint8 pos = phb_desc_get_pbr_pos(DATA_DESC_ADN);
        kal_uint16 record_num = phb_ptr->phb_pbr_table[pos][0].record_num;
        for (i = 0; i < sim_iap_record_cnf->record_num; i++)
        {
            record_index = sim_iap_record_cnf->record[i] + (cblk->actual_count - 1) * record_num;
            seek_byte = (record_index - 1) / 8;
            seek_bit = (record_index - 1) % 8;
            phb_ptr->phb_seek_iap_bitmap[seek_byte] &= ~(1 << seek_bit);
            kal_trace(TRACE_INFO, FUNC_PHB_SIM_SEARCH_IAP_RECORD_CNF, seek_byte, phb_ptr->phb_seek_iap_bitmap[seek_byte]);
        }
    }
        ...
        cblk->actual_count <= phb_ptr->data_desc[DATA_DESC_PBR].record_num)
    {
        ...
        phb_sim_search_record(cblk, DATA_DESC_IAP);
    }
    else
    {
        phb_ptr->phb_seek_done = KAL_TRUE;
        /* phb_startup_handler */
        (*cblk->controller) (NULL, cblk);
    }
}
...
static kal_bool phb_search_iap_record_req(control_block_type *cblk)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    phb_ptr->phb_seek_done = KAL_FALSE;
    if (phb_ptr->data_desc[DATA_DESC_PBR].record_num > 0 && phb_ptr->data_desc[DATA_DESC_IAP].is_support) // seek for usim only
    {
        cblk->actual_count = 1;
		...
		{
            memset(phb_ptr->phb_seek_iap_bitmap, 0xFF, (ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8);
            phb_sim_search_record(cblk, DATA_DESC_IAP);
            return KAL_TRUE;
		}
    }
...
static void phb_search_adn_record_cnf(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    sim_search_record_cnf_struct *sim_adn_record_cnf;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    sim_adn_record_cnf = (sim_search_record_cnf_struct*) ilm_ptr->local_para_ptr;
    ...
    if (cblk->actual_count <= phb_ptr->data_desc[DATA_DESC_PBR].record_num)
    {
        ...
        kal_uint8 pos = phb_desc_get_pbr_pos(DATA_DESC_ADN);
        kal_uint16 record_num = phb_ptr->phb_pbr_table[pos][0].record_num;
        for (i = 0; i < sim_adn_record_cnf->record_num; i++)
        {
            record_index = sim_adn_record_cnf->record[i] + (cblk->actual_count - 1) * record_num;
            seek_byte = (record_index - 1) / 8;
            seek_bit = (record_index - 1) % 8;
            phb_ptr->phb_seek_adn_bitmap[seek_byte] &= ~(1 << seek_bit);
            kal_trace(TRACE_INFO, FUNC_PHB_SIM_SEARCH_ADN_RECORD_CNF, seek_byte, phb_ptr->phb_seek_adn_bitmap[seek_byte]);
        }
    }
        ...
        cblk->actual_count <= phb_ptr->data_desc[DATA_DESC_PBR].record_num)
    {
        ...
        phb_sim_search_record(cblk, DATA_DESC_ADN);
    }
    else
    {
        phb_ptr->phb_seek_done = KAL_TRUE;
...
static kal_bool phb_search_adn_record_req(control_block_type *cblk)
{
    ...
    /*----------------------------------------------------------------*/
    phb_ptr->phb_seek_done = KAL_FALSE;
    if (phb_ptr->data_desc[DATA_DESC_PBR].record_num > 0) // seek for usim only
    {
        cblk->actual_count = 1;
        cblk->total = phb_ptr->data_desc[DATA_DESC_PBR].record_num;
        if (phb_desc_get_file_path(cblk->primary_ID, 1, (kal_uint8)cblk->actual_count, cblk->path))
		{
            memset(phb_ptr->phb_seek_adn_bitmap, 0xFF, (ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8);
            phb_sim_search_record(cblk, DATA_DESC_ADN);
            return KAL_TRUE;
		}
    }
...
static void phb_search_pbc_record_cnf(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    sim_search_record_cnf_struct *sim_pbc_record_cnf;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    sim_pbc_record_cnf = (sim_search_record_cnf_struct*) ilm_ptr->local_para_ptr;
    ...
    if (cblk->actual_count <= phb_ptr->data_desc[DATA_DESC_PBR].record_num)
    {
        ...
        kal_uint8 pos = phb_desc_get_pbr_pos(DATA_DESC_ADN);
        kal_uint16 record_num = phb_ptr->phb_pbr_table[pos][0].record_num;
        for (i = 0; i < sim_pbc_record_cnf->record_num; i++)
        {
            record_index = sim_pbc_record_cnf->record[i] + (cblk->actual_count - 1) * record_num;
            seek_byte = (record_index - 1) / 8;
            seek_bit = (record_index - 1) % 8;
            phb_ptr->phb_seek_pbc_bitmap[seek_byte] &= ~(1 << seek_bit);
            kal_trace(TRACE_INFO, FUNC_PHB_SIM_SEARCH_PBC_RECORD_CNF, seek_byte, phb_ptr->phb_seek_pbc_bitmap[seek_byte]);
        }
    }
        ...
        cblk->actual_count <= phb_ptr->data_desc[DATA_DESC_PBR].record_num)
    {
        ...
        phb_sim_search_record(cblk, DATA_DESC_PBC);
    }
    else
    {
        phb_ptr->phb_seek_done = KAL_TRUE;
...
static kal_bool phb_search_pbc_record_req(control_block_type *cblk)
{
    ...
    /*----------------------------------------------------------------*/
    phb_ptr->phb_seek_done = KAL_FALSE;
    if (phb_ptr->data_desc[DATA_DESC_PBR].record_num > 0 && phb_ptr->data_desc[DATA_DESC_PBC].is_support) // seek for usim only
    {
        cblk->actual_count = 1;
        cblk->total = phb_ptr->data_desc[DATA_DESC_PBR].record_num;
        if (phb_desc_get_file_path(cblk->primary_ID, 1, (kal_uint8)cblk->actual_count, cblk->path))
		{
            memset(phb_ptr->phb_seek_pbc_bitmap, 0xFF, (ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8);
            phb_sim_search_record(cblk, DATA_DESC_PBC);
            return KAL_TRUE;
		}
    }
...
void phb_get_info(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    while (phb_query_count[curr_sim] >=0 && phb_sim_info_tbl[phb_query_count[curr_sim]].primary_ID)
    {
        ...
        if (phb_sim_info_tbl[phb_query_count[curr_sim]].is_usim == PHB_INFO_READ_USIM ||
            phb_sim_info_tbl[phb_query_count[curr_sim]].is_usim == PHB_INFO_SEARCH_USIM)
        {
...
static void phb_build_pbc(ilm_struct *ilm_ptr, control_block_type *cblk)
{
        ...
        cblk->actual_count < ADN_INDEX_MAX_ENTRIES_COUNT)
    {
        ...
        if (phb_ptr->phb_seek_done)
        {
            kal_uint16 seek_byte = (cblk->record_index - 1) / 8;
            kal_uint16 seek_bit = (cblk->record_index - 1) % 8;
            while ((seek_byte < (ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8) && (phb_ptr->phb_seek_pbc_bitmap[seek_byte] & (1 << seek_bit)) == 0)
            {
                ++cblk->record_index;
                seek_byte = (cblk->record_index - 1) / 8;
                seek_bit = (cblk->record_index - 1) % 8;
    ...
    #ifdef __SIM_SEARCH_RECORD_SUPPORT__
        kal_trace(TRACE_INFO, FUNC_PHB_SIM_SEARCH_RECORD_READ_PBC, phb_ptr->phb_seek_done, cblk->record_index);
    #endif
        phb_issue_IO_read(cblk);
    }
    else
...
static void phb_build_adn(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    else
    {
        ...
        if (phb_ptr->phb_seek_done)
        {
            kal_uint16 seek_byte = (cblk->record_index - 1) / 8;
            kal_uint16 seek_bit = (cblk->record_index - 1) % 8;
            kal_uint16 adn_count = 0;
            kal_uint8 pbr_rec = 0, pbr_pos = phb_desc_get_pbr_pos(DATA_DESC_ADN);
            
            phb_IO_cntx* cntx = phb_IO_get_cntx(cblk);
            while (cblk->record_index <= phb_ptr->data_desc[DATA_DESC_ADN].record_num && ((seek_byte < (ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8) && (phb_ptr->phb_seek_adn_bitmap[seek_byte] & (1 << seek_bit)) == 0))
            {
                while(cblk->record_index > (adn_count + phb_ptr->phb_pbr_table[pbr_pos][pbr_rec].record_num))
                {
                	adn_count += phb_ptr->phb_pbr_table[pbr_pos][pbr_rec].record_num;
		pbr_rec++;
                }
                phb_data_desc_mark_free(
                            DATA_DESC_ADN,
    ...
    #ifdef __SIM_SEARCH_RECORD_SUPPORT__
        kal_trace(TRACE_INFO, FUNC_PHB_SIM_SEARCH_RECORD_READ_ADN, phb_ptr->phb_seek_done, cblk->record_index);
    #endif
        phb_issue_IO_read(cblk);
    }
}
...
#ifdef __SIM_SEARCH_RECORD_SUPPORT__
    else if (phb_sim_info_tbl[phb_query_count[curr_sim]].is_usim == PHB_INFO_SEARCH_USIM)
    {
        (*cblk->controller) (NULL, cblk);
        return;
    }
