...
/* under construction !*/
        ...
        if (((access_id & 0x0F) < PHB_CONCURRENCY_DEGREE) &&
            ...
            control_block->res_msg_id == ilm_ptr->msg_id)
        ...
        else
        {
            if(control_block)
            {
                kal_trace(TRACE_INFO, INFO_PHB_WRONG_RES_IF_CB, control_block->ID, control_block->res_msg_id, control_block->IO_cnf_receive);
            }
	        kal_trace(TRACE_INFO, INFO_PHB_WRONG_RES_IF_NOT_CB, access_id, ilm_ptr->msg_id);
