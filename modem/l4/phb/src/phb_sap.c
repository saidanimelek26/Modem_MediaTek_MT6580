...
/*****************************************************************************
 * FUNCTION
 *  l4c_phb_adn_entry_is_free
 * DESCRIPTION
 *  This is for L4C to check if ADN entry is free
 * PARAMETERS
 *
 * RETURNS
 * 
 *****************************************************************************/
kal_bool l4c_phb_adn_entry_is_free(kal_uint16 index)
{   
    free_status_enum result;
    
    result = phb_data_desc_is_free(DATA_DESC_ADN, index);

    if (result == RECORD_FREE)
        return KAL_TRUE;
    else
        return KAL_FALSE;
}
