...
kal_bool phb_sim_file_info(control_block_type *control_block, sim_info_type_enum info_type)
{
    ...
    if (control_block->IO_stage == IO_NONE)
    {
        ...
        control_block->res_msg_id = MSG_ID_SIM_FILE_INFO_CNF;
...
kal_bool phb_sim_search_record(control_block_type *control_block, data_desc_enum type)
{
    ...
    if (control_block->IO_stage == IO_NONE)
    {
        ...
        control_block->res_msg_id = MSG_ID_SIM_SEARCH_RECORD_CNF;
...
static void phb_sim_search_record_cnf(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    if ((sim_search_record_cnf->result == SIM_CMD_SUCCESS) 
				|| ((sim_search_record_cnf->result == SIM_RECORD_NOT_FOUND) && (sim_search_record_cnf->file_idx == FILE_G_ADN_IDX)))
    {
        if (control_block->IO_stage == IO_WAIT)
        {
            /* success */
            ...
            /* data_desc_enum type, */ kal_uint8 chained_id)
{
    ...
    if (phb_sim_prepare_ln_data_to_write(control_block, ef_buffer, chained_id) == KAL_FALSE)
    {
     ...
     control_block->res_msg_id = MSG_ID_SIM_WRITE_CNF;
...
kal_bool phb_sim_read_ln(control_block_type *control_block)
{
    ...
    if (control_block->IO_stage == IO_NONE)
    {
        ...
        control_block->res_msg_id = MSG_ID_SIM_READ_CNF;
        phb_sim_read_record(
            control_block->primary_ID,
            control_block->record_index,
            control_block->length,
...
kal_bool phb_IO_sim_read(phb_IO_cntx* cntx)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    control_block_type *control_block = cntx->control_block;
#ifdef __PHB_USIM_SUPPORT__
    kal_bool result;
#endif
