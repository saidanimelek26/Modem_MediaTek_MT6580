...
kal_uint16 PHB_CB_occupied = PHB_CONCURRENCY_DEGREE - 1;
/*****************************************************************************
 * FUNCTION
 *  phb_context_reset
 * DESCRIPTION
...
control_block_type *phb_alloc_control_block()
{
    ...
    for (i = 0; i < PHB_CONCURRENCY_DEGREE; ++i)
    {
		...
		PHB_CB_occupied = (PHB_CB_occupied + 1) % PHB_CONCURRENCY_DEGREE;
		
	        if (phb_ptr->control_block[PHB_CB_occupied].is_allocated == KAL_TRUE)
        {
            continue;
        }

	        control_block = &phb_ptr->control_block[PHB_CB_occupied];

	        phb_control_block_construct(control_block, PHB_CB_occupied);
        control_block->is_allocated = KAL_TRUE;

		kal_trace(TRACE_INFO, INFO_PHB_NEW_CB_OCCUPIED, PHB_CB_occupied, control_block->ID);
        break;
