...
extern l4_addr_bcd_struct *fdn_num_ptr;
extern l4_name_struct *fdn_name_ptr;

extern kal_uint16 phb_se_get_record_index(phb_type_enum type, kal_uint16 index);
#endif /* __PHB_FDN_GET_NAME__ */

#if defined(__ECALL_SUPPORT__)
extern l4_addr_bcd_struct *sdn_num_ptr;
#endif /* __PHB_FDN_GET_NAME__ */
...
void phb_search_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    if (ilm_ptr != NULL)
    {
                                                       ...
                                                       (l4cphb_search_req->pattern_tag == PHB_BYNUMBER)))
        {
             ...
             */
                // TODO: need to add storage in search request
    ...
    else
    {
        ...
        if (control_block->proc_stage == SEARCH_CONTINUE)
        {
          ...
          */
             ...
             * is not implemented .
             */
                pdu_ptr = get_peer_buff_pdu(control_block->peer_buf_ptr, &pdu_length);
        ...
        l4_addr_bcd_struct *given_pattern)
{
    ...
    */
    ...
    l4cphb_alloc_peer_buf(&peer_buf_ptr, 1);
...
/*****************************************************************************
 * FUNCTION
 *  phb_fdn_get_name_confirm
 * DESCRIPTION
 *  This is phb_fdn_get_name_confirm function of PHB module.
 * PARAMETERS
 *  result              [IN]        
 *  actual_count        [IN]        
 *  src_id              [IN]        
 ...
 *  void
 *****************************************************************************/
static void phb_fdn_get_name_confirm(
                phb_errno_enum result,
                kal_uint16 actual_count,
                kal_uint8 src_id,
                msg_type cnf_msg_id,
                control_block_type *control_block)
{
    ...
    l4cphb_fdn_get_name_cnf_struct *l4cphb_fdn_get_name_cnf;

    l4cphb_fdn_get_name_cnf = (l4cphb_fdn_get_name_cnf_struct*) construct_local_para(
                                                        sizeof(l4cphb_fdn_get_name_cnf_struct),
                                                        TD_CTRL);
    kal_mem_set((kal_uint8*) &l4cphb_fdn_get_name_cnf->alpha_id, 0x00, sizeof(l4_name_struct));
    
    local_param_ptr = (local_para_struct*) l4cphb_fdn_get_name_cnf;

    //l4cphb_fdn_get_name_cnf->total = actual_count;
    l4cphb_fdn_get_name_cnf->src_id = src_id;
    l4cphb_fdn_get_name_cnf->result = result;

    /* Field `cause' is meaningful when I/O occured by using control blocks */
    l4cphb_fdn_get_name_cnf->cause = control_block->cause;
    ...
    if (result != PHB_ERRNO_BUSY)
    {
        ...
        kal_mem_cpy((kal_uint8*) &l4cphb_fdn_get_name_cnf->alpha_id, (kal_uint8*) &control_block->temp_entry.alpha_id, sizeof(l4_name_struct));

        phb_free_control_block(control_block);
    }

    phb_send_ilm(MOD_L4C, MSG_ID_L4CPHB_FDN_GET_NAME_CNF, local_param_ptr, NULL);
    return;
}
...
/*****************************************************************************
 * FUNCTION
 *  phb_fdn_get_name_err_handler
 * DESCRIPTION
 *  This is phb_fdn_get_name_err_handler function of PHB module.
 * PARAMETERS
 *  ilm_ptr             [IN]        The primitives
 *  control_block       [?]         
 * RETURNS
 *  void
 *****************************************************************************/
static void phb_fdn_get_name_err_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  phb_fdn_get_name_handler
 * DESCRIPTION
 *  This is phb_fdn_get_name_handler function of PHB module.
 *  Handles write request from L4C.
 * PARAMETERS
 *  ilm_ptr             [IN]        The primitives
 *  control_block       [?]         
 ...
 *  void
 *****************************************************************************/
void phb_fdn_get_name_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (ilm_ptr != NULL)
    {
        l4cphb_fdn_get_name_req_struct *l4cphb_fdn_get_name_req;

        l4cphb_fdn_get_name_req = (l4cphb_fdn_get_name_req_struct*) ilm_ptr->local_para_ptr;

        /* Cannot allocate, means that PHB is still operating. Reject request! */
        control_block = phb_alloc_control_block();
        if (control_block == NULL)
        {
            phb_fdn_get_name_confirm(PHB_ERRNO_BUSY, 0, l4cphb_fdn_get_name_req->src_id, MSG_ID_INVALID_TYPE, control_block);
            return;
        }

        control_block->src_id = l4cphb_fdn_get_name_req->src_id;
        ...
        if (phb_ptr->state == PHB_STATE_READY)
        {
            name_num_index_type *name_num_index = &phb_ptr->indices.fdn_index;
            phb_entry_struct *phb_entry = &control_block->temp_entry;
            kal_uint16 i, record_index,total_num;
            phb_errno_enum result;

            kal_mem_cpy((kal_uint8*) &phb_entry->tel_number, (kal_uint8*) &l4cphb_fdn_get_name_req->number, sizeof(l4_addr_bcd_struct));
            total_num = name_num_index->data_entry_table.used_count;
            
            for (i = 0; i < total_num; i++)
            {
                if ((record_index = phb_se_get_record_index(PHB_FDN, i + 1)) != (kal_uint16) PHB_INVALID_VALUE)
                {
                    if (phb_compare_bcd_for_wild_char(fdn_num_ptr[record_index - 1].addr_length, phb_entry->tel_number.addr_length, fdn_num_ptr[record_index - 1].addr_bcd, phb_entry->tel_number.addr_bcd) == KAL_TRUE)
                    {
                        phb_entry->index = i;
                        phb_entry->record_index = record_index;
                        control_block->actual_count = 1;
                        if ((fdn_name_ptr + (record_index - 1))->name_length != 0)
                        {
                            kal_mem_cpy((kal_uint8*) &phb_entry->alpha_id, (kal_uint8*) (fdn_name_ptr + record_index - 1), sizeof(l4_name_struct));
                            break;
                        }
                    }
                }
            ...
            if (control_block->actual_count == 1)
            {
                result = PHB_ERRNO_SUCCESS;
            }
            else
            {
                result = PHB_ERRNO_FAIL;
            }
    ...
    {
        phb_fdn_get_name_err_handler(NULL, control_block);
        return;
    }
}
...
phb_errno_enum phb_fdn_get_first_two_number(l4_addr_bcd_struct *digit1, l4_addr_bcd_struct *digit2)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        name_num_index_type *name_num_index = &phb_ptr->indices.fdn_index;
        kal_uint16 i, record_index;
        phb_errno_enum result = PHB_ERRNO_FAIL;

        if (digit1 && digit2 && name_num_index->data_entry_table.used_count >= 2)
        {
            kal_mem_set((kal_uint8*) digit1, 0x00, sizeof(l4_addr_bcd_struct));
            kal_mem_set((kal_uint8*) digit2, 0x00, sizeof(l4_addr_bcd_struct));

            for (i = 0; i < name_num_index->data_entry_table.used_count; i++)
            {
                if ((record_index = phb_se_get_record_index(PHB_FDN, i + 1)) != (kal_uint16) PHB_INVALID_VALUE)
                {
                    if (record_index == 1)
                    {
                        kal_mem_cpy((kal_uint8*) digit1, (kal_uint8*) fdn_num_ptr, sizeof(l4_addr_bcd_struct));
                    }

                    if (record_index == 2)
                    {
                        kal_mem_cpy((kal_uint8*) digit2, (kal_uint8*) (fdn_num_ptr + 1), sizeof(l4_addr_bcd_struct));
                    }
                }
            }
            result = PHB_ERRNO_SUCCESS;
    ...
    {
        return PHB_ERRNO_NOT_READY;
    }
}


phb_errno_enum phb_sdn_get_last_two_number(l4_addr_bcd_struct *digit1, l4_addr_bcd_struct *digit2)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (phb_ptr->state == PHB_STATE_READY)
    {
        ...
        if (digit1 && digit2 && phb_data_desc_get_record_num(DATA_DESC_SDN) >= 2)
        {
            kal_mem_cpy((kal_uint8*) digit1, (kal_uint8*) sdn_num_ptr, sizeof(l4_addr_bcd_struct));
            kal_mem_cpy((kal_uint8*) digit2, (kal_uint8*) (sdn_num_ptr + 1), sizeof(l4_addr_bcd_struct));
            result = PHB_ERRNO_SUCCESS;
        }
