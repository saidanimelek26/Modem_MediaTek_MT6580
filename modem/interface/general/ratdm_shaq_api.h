 ...
 * $Log$
 *
 * 03 04 2013 johnson.liu
 * [MOLY00009204] [Approved patch][1st Round][MT6589TDV1_Phone][CMCC Case][FT][BJ]5.2]_?23?_11:46:38_??????_A??
 * [TDD]Check RAB state before triggering service request.
 *
 ...
 * 
 * 08 31 2012 andrew.wu
 * [MOLY00002965] [MT6583] Fix GPRS related build error
 * <saved by Perforce>
 * 
...
#define M_SHAQ_MAX_Q_SIZE   (((MAX_EXT_PDP_CONTEXT)+(MAX_INT_PDP_CONTEXT))+((MAX_CS_TM_Q_NUM)+(MAX_CS_UM_Q_NUM))+(1))
#define M_SHAQ_MINIMUM_CONTEXT_ID (1)
#define M_SHAQ_MAXIMUM_CONTEXT_ID (((MAX_EXT_PDP_CONTEXT)+(MAX_INT_PDP_CONTEXT))+((MAX_CS_TM_Q_NUM)+(MAX_CS_UM_Q_NUM)))

#define M_MIN_PS_SHAQ_NUMBER (M_SHAQ_MINIMUM_CONTEXT_ID)
#define M_MAX_PS_SHAQ_NUMBER ((MAX_EXT_PDP_CONTEXT)+(MAX_INT_PDP_CONTEXT))
...
#else
kal_bool SHAQ_check_pdp_rab_state(void);
#endif
#endif
