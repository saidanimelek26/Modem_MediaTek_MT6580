...
#if (defined(__PRI_RESEL_SUPPORT__) || defined(__CSG_SUPPORT__))
...
#endif /* defined(__PRI_RESEL_SUPPORT__) || defined(__CSG_SUPPORT__) */

#ifdef __PRI_RESEL_SUPPORT__
//Dedicated Priority Info
#define SCSI_MAX_NUM_OF_PRIO_INFO 8
...
#define SCSI_MAX_NUM_OF_LTEFDD_PRIO_EUARFCN  32 // 32 frequency
...
typedef struct{
   ...
   * for LTEFDD,  every U16 element represent an EUARFCN.
   **/
  union{
    ...
    kal_uint16 *EUARFCN_list; // SCSI_MAX_NUM_OF_LTEFDD_PRIO_EUARFCN <== each of them is two bytes.
  }choice;
...
#define SCSI_MAX_BYTE_OF_UMTS_PSC       64    // bitmask, total 64 bytes.
...
typedef struct{
  ...
  kal_uint16     uarfcn;
  kal_uint8       PSC_bitmap[SCSI_MAX_BYTE_OF_UMTS_PSC]; //every bit represenet a PSC (from 0~512); LSB of octet 1 represent psc = 0, MSB of octet 1 represent psc = 7
...
extern void Scsi_ComposeUmtsGsmBandMask(kal_uint32 UBandBitmask, kal_uint8 GBandBitmask);

extern void Scsi_ComposeUmtsBandDefinitionInfo(void);

extern void Scsi_retrieveUmtsBandListInfo(SCSI_UMTS_BandInfoList **ppScsi_StoredUmtsFddBandListInfo);

extern void Scsi_retrieveUmtsBandDefinedInfo(SCSI_UMTS_BandInfoDefinedList **ppScsi_StoreUmtsFddBandDefinedInfo);

extern void Scsi_retrieveUmtsGsmBandBitmask(kal_uint32 *pUBandBitmask, kal_uint8 *pGBandBitmask);

extern kal_bool Scsi_ensureUarfcnValid(kal_uint16 uarfcn);
...
extern kal_bool Scsi_UarfcnValidateInAllSupportUmtsBand(kal_uint16 uarfcn, SCSI_UMTS_UarfcnDirectionType type, SCSI_UMTS_Band_UARFCN_Identifier *band_id);
...
#ifdef __UMTS_FDD_MODE__
extern kal_bool Scsi_UarfcnValidateInAllSupportUmtsBandExt(kal_uint16 uarfcn, SCSI_UMTS_UarfcnDirectionType type, SCSI_UMTS_Band_UARFCN_Identifier *band_id, kal_uint8 *indexInBandFDDList);
#endif

extern kal_uint8 Scsi_GetNumOfUmtsSupportBand(void);

extern void Scsi_GetUmtsSupportBandInfo(SCSI_UMTS_BandInfoList const **workBandInfo);

extern void Scsi_RequiredUmtsBandListQuery(kal_uint8 *query_array);

extern kal_bool Scsi_IsUmtsSupportBand(SCSI_UMTS_Band_UARFCN_Identifier band_id);
...
#ifdef __CSG_SUPPORT__
// put the pSlitInfo which contain this uarfcn in pSplitInfo, if not found, set content of pSplitInfo all zero.
// besides, according to tick_left tick_previous to delete the expired split info.
extern void SCSI_getCSGSplitInfo(SCSI_SIM_INDEX sim_index, kal_uint16 uarfcn, SCSI_csg_split_info * const pSplitInfo);

// put the content of pCSGSplitInfoList in SCSI to pSplitInfoList, if pCSGSplitInfoList is null, set content of pSplitInfoList all zero.
// besides, according to tick_left tick_previous to delete the expired split info.
extern void SCSI_getCSGSplitInfoList(SCSI_SIM_INDEX sim_index, SCSI_csg_split_info_list * const pSplitInfoList);


// If found the same frequecny replace it, else allocate one memory and put the SCSI_csg_split_info in the CSGSplitInfo.
// besides, according to tick_left tick_previous to delete the expired split info.
extern void SCSI_newCSGSplitInfo(SCSI_SIM_INDEX sim_index, kal_uint32 tick_left, SCSI_csg_split_info const * const pSplitInfo);
