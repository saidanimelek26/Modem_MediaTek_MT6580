...
typedef struct {
	...
	kal_bool mm_seq_pending;
	kal_bool tcm_deconfig;
    kal_uint8 nsapi[MAX_EXT_PDP_CONTEXT+1];
} ratdm_rabm_query_statistics_req_struct;
...
typedef struct {
	...
	kal_bool mm_seq_pending;
	kal_bool tcm_deconfig;
    single_nsapi_info_struct query_result[MAX_EXT_PDP_CONTEXT+1];
} ratdm_rabm_query_statistics_rsp_struct;
...
typedef struct {
    ...
    kal_uint8 serial_num;
    kal_uint8 total_nsapi;
    single_nsapi_reset_cmd_struct reset_cmd[MAX_EXT_PDP_CONTEXT+1];
...
typedef struct {
    ...
    kal_uint8 total_nsapi;
	single_nsapi_reset_cmd_struct reset_cmd[MAX_EXT_PDP_CONTEXT+1];	
    single_nsapi_info_struct reset_result[MAX_EXT_PDP_CONTEXT+1];
} ratdm_pdcp_reset_statistics_rsp_struct;

typedef struct {
    LOCAL_PARA_HDR
    kal_uint8 total_nsapi_id;
    single_nsapi_info_struct stat[MAX_EXT_PDP_CONTEXT+1];
} ratdm_rabm_statistics_ind_struct;

typedef struct {
    LOCAL_PARA_HDR
    kal_uint8 total_nsapi_id;
    single_nsapi_info_struct set_cmd[MAX_EXT_PDP_CONTEXT+1];
} ratdm_rabm_set_statistics_req_struct;

typedef struct {
    LOCAL_PARA_HDR
    kal_uint8 total_nsapi_id;
    single_nsapi_set_info_struct set_result[MAX_EXT_PDP_CONTEXT+1];
} ratdm_rabm_set_statistics_cnf_struct;
