...
#ifdef __UMTS_RAT__
#define UMTS_GSM_MEAS_MAX_CELL_NUM (32)// ref to MPH_UMTS_GSM_MEAS_MAX_CELL_NUM
#define UMTS_GSM_MAX_STANDBY_BSIC_NUM (32)// ref to MPH_UMTS_GSM_MEAS_MAX_CELL_NUM
#endif
...
typedef struct {
    ...
    kal_uint8 usf_granularity;
    usf_value_struct usf[8];
    pdtch_ch_ctrl_struct pdtch_chan_params;
    kal_uint8 direction;
...
typedef struct {
    ...
    kal_uint8 direction;
    kal_bool is_all_tbf_rel;
    kal_uint8 identity; /* Evelyn 20090115 : add identity for L1 */
} rr_mpal_pdtch_disconn_req_struct;
#endif/*__NEW_TBF_ARCH__*/
...
#endif
    kal_uint8 list_len;
    kal_uint16 cell_list[UMTS_GSM_MEAS_MAX_CELL_NUM];
} rr_mpal_umts_gsm_meas_req_struct;
...
typedef struct {
    ...
    kal_uint8 list_len;
    kal_uint16 cell_list[UMTS_GSM_MEAS_MAX_CELL_NUM];
    kal_uint16 period_in_second;         /* 60*N */
} rr_mpal_umts_gsm_prio_meas_req_struct;
...
} mpal_dual_gsm_bsic_read_struct;

typedef struct
{
   LOCAL_PARA_HDR
   /* The number of valid element in bsic_read_list */
   kal_uint8                              list_len;
   /* The database for BSIC read */
   mpal_dual_gsm_bsic_read_struct         cell_bsic_list[UMTS_GSM_MAX_STANDBY_BSIC_NUM];
} rr_mpal_umts_gsm_bsic_read_req_struct;

typedef struct
{
   LOCAL_PARA_HDR
   /* The number of valid element in bsic_stop_list */
   kal_uint8                              list_len;
   /* The database for BSIC stop */
   kal_uint16                             stop_bsic_list[UMTS_GSM_MAX_STANDBY_BSIC_NUM];
} rr_mpal_umts_gsm_bsic_stop_req_struct;
...
} rr_mpal_umts_handover_req_struct;
#endif /* end of __PS_HO__ */
