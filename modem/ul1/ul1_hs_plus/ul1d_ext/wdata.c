...
#if IS_MIPI_PA
kal_uint16 mipi_offset  = (MAX_OFFSET - MIPI_OFFSET);
#endif
#if IS_3G_MIPI_SUPPORT
...
#if IS_MIPI_PA
kal_int16  UMTS_TX_START_TQ_TABLE[9] =
{
   ...
   /* TQ_SLOT_BEGIN(i) + */ -TC_MIPI_ST1,
   /* TQ_SLOT_BEGIN(i) + */ -TC_MIPI_ST2  
};


__SECTION_INTSRAM_RW__
kal_int16  UMTS_TX_END_TQ_TABLE[4] =
{
   /* TQ_SLOT_BEGIN(i) + */ TC_ST3,
   /* TQ_SLOT_BEGIN(i) + */ TC_PT3,
   /* TQ_SLOT_BEGIN(i) + */ TC_PT3A,
   /* TQ_SLOT_BEGIN(i) + */ TC_MIPI_ST3
};
#else
...
kal_uint8 band19_ch_sel = BAND10_CHANNEL_SEL;
...
#ifdef UL1CUSTOM_MEMORY_ENABLE
kal_bool ul1custom_memory_enable = UL1CUSTOM_MEMORY_ENABLE;
#else
kal_bool ul1custom_memory_enable = 0;
#endif
...
#if defined (IS_MT6580_EVM_RCDE_IMPROVE)
   #if IS_MT6580_EVM_RCDE_IMPROVE
   kal_bool is_MT6580_EvmPeak_workaround = KAL_TRUE;
   #else
   kal_bool is_MT6580_EvmPeak_workaround = KAL_FALSE;
   #endif
#else
kal_bool is_MT6580_EvmPeak_workaround = KAL_FALSE;
#endif

#if defined (IS_MT6166_EVM_RCDE_IMPROVE)
   #if IS_MT6166_EVM_RCDE_IMPROVE
   kal_bool is_MT6166_EvmPeak_workaround = KAL_TRUE;
   #else
   kal_bool is_MT6166_EvmPeak_workaround = KAL_FALSE;
   #endif
#else
kal_bool is_MT6166_EvmPeak_workaround = KAL_FALSE;
#endif
