 ...
 * [MOLY00006323] [L1D][Modify] Modify MT6589 MD2 saturation point and noise floor
 * .
 *
 * 10 29 2012 ola.lee
...
void L1I_SetAFcData( int16 calibrated_dac_default, intx calibrated_int_slope )
{
   ...
   if( calibrated_int_slope == 0 )
   {
      ...
      extern unsigned short L1D_DualLoopAFC_GPSCoClk_Check(void);
      if( (RFSpecialCoef.rx.fixafc.fixafc_en || L1D_DualLoopAFC_GPSCoClk_Check()) && !L1D_CheckIfMetaMode() )
   #else
   ...
   #endif
      {
         ...
         // Force turn on GPS Co-Clock relate buffer when Fix AFC enable
         extern unsigned char  CLK_EN_TABLE[4];
         if( GPS_CO_CLOCK_BUF_IDX>=1 && GPS_CO_CLOCK_BUF_IDX<=(sizeof(CLK_EN_TABLE)/sizeof(unsigned char)) )
         {  CLK_EN_TABLE[GPS_CO_CLOCK_BUF_IDX-1]=1;  }
   #endif
      }
      else
      {
