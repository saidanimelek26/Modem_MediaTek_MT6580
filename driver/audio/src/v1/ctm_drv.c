...
#else // origial feature phone / modem only

#ifdef L1CTM_DEBUG_MODE
...
/*RecordDSP*/ #if defined(MT6252) || defined(MT6252H) || defined(MT6236) || defined(MT6236B) 
...
{
	(void)pcm_ex_vm_counter;
// this function should be modified as based on SAL, 2012.11.15 wayne
/*
   ...
   if( sc_mode <= 2 ) 
   {
      vm_control =((*DSP_RX_TCH_S(0,0) & 0x3E) << 10) |
...
void l1ctm_in_hisr()
{
   ...
   if (CtmReadFileDelay == 20)
   {
      if (cosim_guard_period > 0)
      {
         ...
         if (cosim_guard_period == 0)
         {
            //kal_prompt_trace(MOD_L1SP, "Turning on COSIM bit in DP_CTM_COSIM_CONTROL");
            kal_dev_trace(TRACE_STATE, L1CTM_FAKE_COSIM);
            bfi_data = DP_CTM_COSIM_CONTROL;
            DP_CTM_COSIM_CONTROL = bfi_data | 0x8000;
         }
      }
   }
#endif
   ...
   if(l1ctm->pcm_fifo_read == l1ctm->next_to_process){
      //kal_prompt_trace(MOD_L1SPHISR,"l1ctm buffer overflow");
...
//       ASSERT(l1ctm->dl_pcm_fifo[buf_idx][I] == 0);
   ...
   if (cosim_guard_period > 0)
   {
      ...
      {
         //kal_prompt_trace(MOD_L1SP, "Turning on COSIM bit in DP_CTM_COSIM_CONTROL");
         kal_dev_trace(TRACE_STATE, L1CTM_COSIM);
         bfi_data = DP_CTM_COSIM_CONTROL;
         DP_CTM_COSIM_CONTROL = bfi_data | 0x8000;
      }
   }
   ...
   kal_dev_trace(TRACE_INFO, L1CTM_BFI_INFO, buf_idx, bfi_store_idx, dsp_tch_bfi, l1ctm->l1d_bfi_info[buf_idx]);
...
//                             dlInBfiReadFlag, dlInBfiReadCount, dlInBfiDataCount, dlInBfiReadBufIdx);
      ...
      if((cprm_debug_flag & CPRM_MASK_REC_UL_PCM_IN) && sdCardNotFullFlag && f_ul_in_pcm_rec_hdl > 0) 
      {
         ...
         {            
            ...
            L1Audio_InProcCall(MOD_L1SP, med_integrate_debug_hdlr, UL_IN_WRITE_FILE_REQUEST, (void*)write_msg_info);                                                                           
		...
		#elif defined(UL_IN_PCM_RECORD_MD)
      {
         ...
         if( l1ctm->debugCallback != NULL ){
				l1ctm->debugCallback(UL_IN_WRITE_FILE_REQUEST, l1ctm->ul_pcm_fifo[buf_idx], 160, false);
            // ptr = l1ctm->ul_pcm_fifo[buf_idx];
		...
		#elif defined(DL_IN_PCM_RECORD_MD)
      {
         // uint16 *ptr;
         if( l1ctm->debugCallback != NULL ){
				l1ctm->debugCallback(DL_IN_WRITE_FILE_REQUEST, l1ctm->dl_pcm_fifo[buf_idx], 160, false);
            // ptr = l1ctm->dl_pcm_fifo[buf_idx];
            // spc_CTMWriteToMDBuffer( (kal_uint8 *)ptr, 320, KAL_FALSE);
         }
      }
      ...
      #ifdef DL_IN_BFI_RECORD
      dlInBfiWriteBuf[dlInBfiWriteBufIdx][dlInBfiWriteCount++] = (!SAL_CTM_IsIdle()<< 6) + (l1ctm->l1d_bfi_info[buf_idx]<<5) + l1ctm->bfi_info[buf_idx];
...
//#endif         
		...
		#elif defined(UL_OUT_PCM_RECORD_MD)
      {
         ...
         if( l1ctm->debugCallback != NULL ){
				if(l1ctm->ul_mute_flags & (1 << l1ctm->next_to_process)) {
					l1ctm->debugCallback(UL_OUT_WRITE_FILE_REQUEST, zero_dl_pcm, 160, false);
				} else {         
					l1ctm->debugCallback(UL_OUT_WRITE_FILE_REQUEST, l1ctm->ul_pcm_fifo[buf_idx], 160, false);
				}
            // ptr = l1ctm->ul_pcm_fifo[buf_idx];
            // spc_CTMWriteToMDBuffer( (kal_uint8 *)ptr, 320, KAL_FALSE);
         }
		...
		#elif defined(DL_OUT_PCM_RECORD_MD)
      {
         ...
         if( l1ctm->debugCallback != NULL ){
				if(l1ctm->dl_mute_flags & (1 << l1ctm->next_to_process)) { 
					l1ctm->debugCallback(DL_OUT_WRITE_FILE_REQUEST, zero_dl_pcm, 160, false);
				} else {
					l1ctm->debugCallback(DL_OUT_WRITE_FILE_REQUEST, l1ctm->dl_pcm_fifo[buf_idx], 160, false);
				}
            // ptr = l1ctm->dl_pcm_fifo[buf_idx];
...
//         L1Audio_InProcCall(med_integrate_debug_hdlr, DSP_REC_EQ_SO0_REQUEST, (void*)write_msg_info);
      }
      #endif // RECORD_DSP_RXBUFFER
...
/* under construction !*/
   ...
   // idea: use bits to handle the buffer mute, 
   // l1ctm->ul_mute_flags = 2^PCM_FIFO_LEN - 1; // pow(2, PCM_FIFO_LEN) -1
...
//       fh_dsp_rec_eq_so0 = FS_Open(file_name,  FS_CREATE_ALWAYS);
   ...
   //if(ctm_param != NULL)
   //{
      //DP_CTM_AMRCODEBOOK_GAIN_LIMIT = CPRM_CODEBOOK_GAIN_THRESHOLD;
      //DP_CTM_AMRCODEBOOK_GAIN_UPDATE = CPRM_CODEBOOK_GAIN_VALUE;
   //}
   ...
   if (CtmReadFileDelay == 20)
   {
      ...
      kal_trace(TRACE_STATE, L1CTM_COSIM_MODE);
