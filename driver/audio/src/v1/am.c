 ...
 *
 * 03 23 2017 ting-ni.chen
 * [MOLY00235068] [Rose][non-SIP][Duplicating to ALPS02851488]The volume of the voice output from headset will be louder when turn on airplane mode.(5/5)
 * Close BGS directly when EPOF
 *
 * 03 17 2017 ting-ni.chen
 * [MOLY00235068] [Rose][non-SIP][Duplicating to ALPS02851488]The volume of the voice output from headset will be louder when turn on airplane mode.(5/5)
 * .
 *
 * 07 11 2015 ys.hsieh
 * [MOLY00081955] notify codec to L4C
 * .notify codec to L4C again
 *
 * 07 10 2015 ys.hsieh
 * [MOLY00081955] notify codec to L4C
 * 	notify L4C codec roll back
 *
 * 07 10 2015 ys.hsieh
 * [MOLY00081955] notify codec to L4C
 * notify codec to L4C
 ...
 * .
 *
 * 03 03 2014 thomas.chen
 * [MOLY00058059] [97055]2G模式，?牙耳机切?手持模式后送?音明?降低
 * 	.
 *
 * 12 25 2013 sheila.chen
 * [MOLY00051367] [MT6582] NXP Feature Check in
 * NXP phone call setting (new feature)
 *
 * 10 11 2013 lanus.chao
 * [MOLY00040818] [MT6571]SPE DDL modification
 * .
 *
 * 10 11 2013 lanus.chao
 * [MOLY00040818] [MT6571]SPE DDL modification
 * 1. 6571 TDDNC/ DMNR DDL seperate
 * 2. enable SPE/BGSND on 6571
 *
 * 10 08 2013 thomas.chen
 * [MOLY00040462] the speech driver supports TCH_Notify even in 2G only project
 * 	[MOLY00040462] speech driver support TCH_Notify even in 2G only project
 * 	.
 *
 * 07 11 2013 sheila.chen
 * [MOLY00026590] [Volunteer Patch][MT6582] Loudspeaker mode DMNR
 * loudSpk DMNR for 82. ddload change
 *
 * 05 20 2013 thomas.chen
 ...
 *
 * 04 20 2013 jy.huang
 * [MOLY00020400] Fix the sanity fail . Remove the kal_trace in AM_ENQUEUE
 * .
 *
 ...
 * [MOLY00008234] [MT6572/MT6582] Integration
 * Move dynamic download to speech mode change.
 *
 * 03 07 2013 sheila.chen
 * [MOLY00008234] [MT6572/MT6582] Integration
 * Lenovo feature change on MT6572 & 6582, Magic concall related
 *
 * 03 04 2013 yt.liou
 * [MOLY00010340] DMNR modify for CMCC and DMNR calibration -- mcu part
 * .
 *
 ...
 * [MOLY00007673] [P188T10_ DVT1][MT6589][CMCC Case][FT][Nanjing][2.1][M]_第59?_13:42:46_手机通???音_B手机
 ...
 *
 * 11 27 2012 wayne.wang
 * [MOLY00002694] [SAL] Speech abstration layer phased in
 * SAL - add vm enh_debug_info and dynamic ctrl
 *
 * 11 23 2012 wayne.wang
 * [MOLY00002694] [SAL] Speech abstration layer phased in
 * vm file split pattern + SAL vm and device log + SAL device setting
 *
 * 11 20 2012 wayne.wang
 * [MOLY00002694] [SAL] Speech abstration layer phased in
 * SAL - add wideband silence pattern 
 *
 * 11 16 2012 sheila.chen
 * [MOLY00000112] [MT6583] Pre-integration
 * Add DL mute function
 *
 * 11 15 2012 sheila.chen
 * [MOLY00000112] [MT6583] Pre-integration
 * Warning removal
 *
 * 11 15 2012 wayne.wang
 * [MOLY00002694] [SAL] Speech abstration layer phased in
 * SAL - clear old sheriff definition
 *
 * 11 14 2012 wayne.wang
 * [MOLY00002694] [SAL] Speech abstration layer phased in
 * SAL - epl + pcmrec concurrency
 *
 * 11 14 2012 sheila.chen
 * [MOLY00000112] [MT6583] Pre-integration
 * idle VM bug fix
 *
 * 11 14 2012 sheila.chen
 * [MOLY00000112] [MT6583] Pre-integration
 * Fix DMNR calibration bug
 *
 * 11 13 2012 sheila.chen
 * [MOLY00000112] [MT6583] Pre-integration
 * fix p4w potential bug
 *
 * 11 13 2012 wayne.wang
 * [MOLY00002694] [SAL] Speech abstration layer phased in
 * SAL - SPH ENH DYNAMIC CTRL
 *
 * 11 08 2012 sheila.chen
 * [MOLY00005929] [Pre-SQC1][Blocking][TE][Call]Mute not work for the call
 * Mute function implement
 *
 * 11 08 2012 wayne.wang
 * [MOLY00002694] [SAL] Speech abstration layer phased in
 * SAL - mute cotrol
 *
 * 11 08 2012 sheila.chen
 ...
 * [MOLY00002694] [SAL] Speech abstration layer phased in
 * SAL - epl plus the refmic pcm logging
 * 
 ...
 * [MOLY00002694] [SAL] Speech abstration layer phased in
 * SAL - dsp init and enhancement
 * 
 * 09 04 2012 sheila.chen
 * [MOLY00000112] [MT6583] Pre-integration
 * add SAL header for remove warning.
 * 
...
static void AM_Enqueue( void (*code)(uint32, uint32), uint32 data, uint32 data2, bool isCritical )
{
   ...
   // kal_brief_trace( TRACE_INFO, AM_ENQUEUE, isCritical); 
...
void AM_WriteFirCoeffs( const int16 in_coeff[45], const int16 out_coeff[45] )
{
   ...
   if( (am.state & AM_STATE_SPEECH) == 0 ) //for the case in speech change speech mode, do not switch IIR
   {
...
#endif
   kal_prompt_trace(MOD_L1SP, "[AM_DSP_SpeechOff] am.state = %d", am.state);
...
#if defined( __UMTS_RAT__ ) || defined( __UMTS_TDD128_MODE__ )
void AM_DSP_InterRAT_W2G( uint32 data , uint32 extraInfo)
{
   (void)data;
   if( !(am.state & AM_STATE_SPEECH) )
      return;
  ...
  L1D_Reset_Time_Drift();    
   ...
   L1Audio_Msg_INTERRAT_W2G(am.speech_mode);
}

void AM_InterRAT_W2G()
{
   AM_Enqueue( AM_DSP_InterRAT_W2G, 0, 0, false );
   AM_FlushQFunction();
}

void AM_DSP_RAT_sub(uint32 codec , uint32 extraInfo)
{
...
void AM_DSP_InterRAT_G2W( uint32 codec , uint32 extraInfo)
{
   AM_DSP_RAT_sub(codec, SAL_APP_TYPE_2GCall);
   L1Audio_Msg_INTERRAT_G2W(codec);
}
void AM_InterRAT_G2W(uint32 codec)
{
   AM_Enqueue( AM_DSP_InterRAT_G2W, codec, 0, false );
   AM_FlushQFunction();
}

void AM_DSP_3G_INTRARAT (uint32 codec, uint32 extraInfo)
{
   AM_DSP_RAT_sub(codec, SAL_APP_TYPE_3GCall);
...
static void AM_DSP_VMRecordOn( uint32 speech_mode , uint32 extraInfo)
{
   ...
   if( am.state & (AM_STATE_SPEECH |AM_STATE_VOIP) ) {       /* Conversation Recording */
...
static void AM_DSP_BluetoothOff( uint32 data , uint32 extraInfo)
{
   ...
   // AFE_TurnOffBluetooth(); // use the following function instead
...
void AM_DSP_PCM_EX_On(uint32 type, uint32 data)
{
   ...
   Sal_PCMEx_Config_t cfg;
	...
	/*
	if(AM_PCMEX_TYPE_DEDICATION != type && AM_PCMEX_TYPE_DACA_DEDICATION != type){ 
	   *AFE_VLB_CON |= (0x01<<3);  // Bypass hardware IIR filter 
	}
	*/
...
void AM_DSP_PCM_EX_Off(uint32 type, uint32 data)
{
   ...
   switch(type)
   {
	...
	/*
#if defined(__DUAL_MIC_SUPPORT__) || defined(__SMART_PHONE_MODEM__)
		if(type == AM_PCMEX_TYPE_REC_ONLY_CAL || type == AM_PCMEX_TYPE_REC_PLAY_CAL) {
			cfg->DMNR_cal = false;
			SAL_ENH_SetULPath(false);
...
#if (SPE_LOUDSPK_DMNR) || (DSP_DYNAMDOWNLOAD_TDNC_DMNR_EXOR)	
    // for calibration usage, need agc only
