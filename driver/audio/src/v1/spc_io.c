...
void spcIO_copyDataToMdShareBuf(const kal_uint8 *pSrc, const kal_int16 srcLen)
{
	...
	kal_prompt_trace(MOD_L1SP, "[spcIO_copyDataToMdShareBuf 1] gSpcIo.mdShareBufWrite = %d, srcLen = %d", gSpcIo.mdShareBufWrite, srcLen);
	...
	if((curOffset + srcLen) > gSpcIo.mdShareBufSize) {
		...
		kal_uint8 * p1 = (kal_uint8 *)(gSpcIo.pMdShareBuf + curOffset);
		kal_uint8 * p2 = (kal_uint8 *)(gSpcIo.pMdShareBuf + curOffset+1);
		kal_uint8 * p3 = (kal_uint8 *)(gSpcIo.pMdShareBuf + curOffset+2);

		kal_prompt_trace(MOD_L1SP, "[spcIO_copyDataToMdShareBuf 2] %d %d %d", *p1, *p2, *p3);


		segment = srcLen - segment;
		kal_prompt_trace(MOD_L1SP,"[spcIO_copyDataToMdShareBuf xx] segment = %d", segment);
		...
		kal_prompt_trace(MOD_L1SP, "[spcIO_copyDataToMdShareBuf 3] %d %d %d", *p1, *p2, *p3);
	...
	kal_prompt_trace(MOD_L1SP, "[spcIO_copyDataToMdShareBuf 4] gSpcIo.mdShareBufWrite = %d", gSpcIo.mdShareBufWrite);
	...
	SPCIO_MSG_FROM_SPC spcIoMsg)
{
  ...
  kal_prompt_trace(MOD_L1SP, "[SpcIO_WriteDataToAp] srcLen1 = %d, srcLen2 = %d", srcLen1, srcLen2);
	...
	// mapper 
	msg = spcIO_msgMapper(spcIoMsg);
	kal_prompt_trace(MOD_L1SP, "[SpcIO_WriteDataToAp] length = %d, offset = %d", length, offset);	
...
void SpcIO_Init(void)
{
	...
	kal_prompt_trace(MOD_L1SP, "[SpcIO_Init] gSpcIo.pMdShareBuf = %d", gSpcIo.pMdShareBuf);
	Spc_Init();
