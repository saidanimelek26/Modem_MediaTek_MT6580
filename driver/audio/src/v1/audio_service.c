...
#if defined(MT6280) // data card like
#define  MAX_HISR_HANDLER        6 // 3g, datacard, vm, epl
#elif defined(MT6589) // smart phone modem
#define  MAX_HISR_HANDLER        8 // 3g, pnw, vm, epl, bgsnd, record
...
#else // no 3g
#if defined(MT6280) // data card like
#define  MAX_HISR_HANDLER        4 // datacard, vm, epl
#elif defined(MT6589) // smart phone modem
#define  MAX_HISR_HANDLER        6 // pnw, vm, epl, bgsnd, record
#elif defined(MT6572) || defined(MT6582) || defined(MT6592) || defined(MT6571)|| defined(MT6580)
#define  MAX_HISR_HANDLER       10 // nw, DACA for bt, vm, epl, bgsnd(ul+dl), record, cvsd
	...
	if(from_sph) {
		...
		for( i=1; i< SAL_DSPINT_PRIO_MAX; i++) {
			if(sph_int & (1 << i)) {
				...
				} else if(SAL_DSPINT_PRIO_WARN_MSG_UL == i || SAL_DSPINT_PRIO_WARN_MSG_DL == i) {
					uint16 ulcode, dlcode;
					SAL_GetWarnMsg(&ulcode, &dlcode);
					L1Audio_Msg_DSP_D2C_SPEECH_WARNING_INT(i, ulcode, dlcode);
				} else {
...
#if defined(MT6280)
/*
   //GPIO Setting for 8904. There exist two kinds of setting

   //the first
   *(volatile uint16 *)(0xA0070000) &= ~0x00F0;  // GPIO direction
   *(volatile uint16 *)(0xA0070000) |=  0x00E0;  // GPIO direction
   *(volatile uint16 *)(0xA0071100) =   0x2222;  // GPIO 4, 5, 6, 7 MODE : DAIPCMIN, DAICLK, DAISYNC, DAIPCMOUT   
		
*/
   //Config_WM8904(MIC_8K_LONGSYNC_PHONE_CALL);
#endif
...
void l1audio_console_handler(kal_char *string)
{
   (void)string;
	...
	if(strcmp(string, "speechOn")==0){
		...
		L1SP_Speech_Off();
	} else if(strcmp(string, "lpOn")==0){
		SAL_LBK_Codec(true);
	} else if(strcmp(string, "lpOff")==0){
		SAL_LBK_Codec(false);
	} else if(strcmp(string, "8kAfeLpOn")==0){
		kal_uint16 value = (*(volatile kal_uint16 *)(0x82207DB6));
		*(volatile kal_uint16 *)(0x82207DB6) = (value|0x2);
	} else if(strcmp(string, "8kAfeLpOff")==0){
		kal_uint16 value = (*(volatile kal_uint16 *)(0x82207DB6));
		*(volatile kal_uint16 *)(0x82207DB6) = (value&(~0x2));
	}
