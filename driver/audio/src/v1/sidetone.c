...
#endif // defined(MT6589) || defined(MT6572) || defined(MT6582) || defined(MT6592) || defined(MT6571)
...
{	
	...
	if(gSidetoneInfo.isActive){
		...
		// HW is Q1.15 Format		
		tempTargetValue = (((kal_uint32)(dspValue* gSidetoneInfo.extStVolume))>>12);
		if(tempTargetValue > 0x7FFF) {
			tempTargetValue = 0x7FFF;
		}
        targetValue = (kal_uint16)(tempTargetValue);
...
{
    kal_uint16 dspValue = 32767, targetValue; //fix dspValue to 32767
    kal_uint32 tempTargetValue;

	if(SIDETONE_SOLUTION_VER_HW == gSidetoneInfo.solutionVer) {
		sidetone_hw_SetExtStVolume(vol);
        tempTargetValue = (((kal_uint32)(dspValue* vol))>>12);
        if(tempTargetValue > 0x7FFF) {
            tempTargetValue = 0x7FFF;
        }
        targetValue = (kal_uint16)(tempTargetValue);
        WriteApReg(AP_AFE_SIDDETONE_GAIN, targetValue);	 // write to ap hw 
...
void SIDETONE_TurnSwSidetoneOn(kal_bool on)
{
	if(SIDETONE_SOLUTION_VER_HW == gSidetoneInfo.solutionVer) {
		...
		ASSERT(0); // please use SIDETONE_SetSolutionVer before turn on software sideton on
	}
}
