 ...
 *
 * 06 29 2015 ting-ni.chen
 * [MOLY00109599] 通?中由?牙模式切?到手柄模式概率性出?接收?音异常?大，引起听筒破音
 * .
 *
...
#define ust_get_duration_true(start, end)   (((end) >= (start))? (((end) - (start))): ((USCNT_WRAP - (start) + (end) + 1)))
...
void AFE_DELAY(uint16 delay)
{
   ...
   }while(delay > ust_get_duration_true(curr_frc, latest_us));
}
