...
* PARAMETERS
*  MPU_DUMP * mpu_dump_output       The buffer for mpu settings dump
*  kal_uint32 rgn_index             region index
*
* RETURNS
*  KAL_TRUE:  for success
*  KAL_FALSE: for failure  
*
*************************************************************************/
kal_bool mpu_dump_region(MPU_DUMP * mpu_dump_output, kal_uint32 rgn_index)
{
    volatile MPU_DUMP * internal_dump_ptr;
    
    internal_dump_ptr = (volatile MPU_DUMP *)mpu_dump_output;
    cp15_mpu_set_rgn_index(rgn_index);
    internal_dump_ptr->region[rgn_index].Region_Base_Address = cp15_mpu_get_rbar();  
    internal_dump_ptr->region[rgn_index].Region_Size_Enable = cp15_mpu_get_rser(); 
    internal_dump_ptr->region[rgn_index].Region_Access_Control = cp15_mpu_get_racr(); 
...
static kal_bool mpu_setup_region(const REGION_SETTING_TYPE * region_setting)
{
    ...
    mpu_dump_region(&ARM_MPU_SETTING, region_setting->rgn_no);
...
{
    kal_uint32 ITCM_base;
    //kal_uint32 DTCM_base;
    
    ITCM_base = custom_get_INTSRAMCODE_Base();
    //DTCM_base = custom_get_INTSRAM_PHYSICAL_BOUNDARY_Base();

    /* Setup address remapping on bus */
    /* 0xF -> 0x0 */
    Bus_Virtual_Mapping_Enable();
    ...
    mpu_set_region_primitive(1, ITCM_base, RGN_SIZE_8MB, MPU_NORMAL_INNC_OUTNC|MPU_PRW_URW, 1); 
...
void MPU_Init(void)
{
    ...
    kal_uint32 ROM_Length = custom_get_1st_ROM_ROMLength();
    ...
    kal_uint32 INTSRAM_Total_Length = custom_get_INTSRAM_PHYSICAL_BOUNDARY_Base() + custom_get_INTSRAM_PHYSICAL_BOUNDARY_MaxSize() - INTSRAM_Code_Base;
    ...
    /* The MPU region setting order is important... for priority */
    /* Set 0x0 - DRAM limit => NC R/W */
    mpu_auto_make_region(0x0, RAM_Limit, MPU_XN | MPU_NORMAL_INNC_OUTNC | MPU_PRW_URW, 1);
    /* Set Cacheable RAM region (0xF...) */
    mpu_auto_make_region(MAP2CREGVA(0x0), RAM_Limit, MPU_NORMAL_INWBWA_OUTWBWA | MPU_PRW_URW, 1);
    ...
    mpu_auto_make_region(MAP2CREGVA(ROM_Base), ROM_Length, MPU_NORMAL_INWBWA_OUTWBWA | MPU_PR_UR, ROM_Protect_Region_Mode);
    
    /* TCM */
    mpu_auto_make_region(INTSRAM_Code_Base, INTSRAM_Total_Length, MPU_NORMAL_INNC_OUTNC | MPU_PR_UR, 1);
    mpu_auto_make_region(INTSRAM_Data_Base, INTSRAM_Data_Length, MPU_NORMAL_INNC_OUTNC | MPU_PRW_URW, 1);
    
    /* Custom Region Framework (APB region should be included in here)*/
    /* [DRIVER Note]
     * Y_MPU_START_REGION_IDX => should be defined as the first region index that custom region CAN use
     * Y_MPU_MAX_REGION_IDX => should be defined as the last region index that custom region CAN use
     * These 2 macros are use to check region number overflow at compile time
     */
#if defined(__MTK_TARGET__)
    ...
    {
        kal_uint32 i,j, protect_start_addr, protect_range;
        EMI_MPU_Clear_Status();
        
        protect_range = custom_get_1st_ROM_RAMBase() - 0x0;
        
        // Protect code region for each wrapping regions
        for(i=0,protect_start_addr=0x0;(protect_start_addr&(CREG_OFFSET))==0;i++,protect_start_addr+=EMI_EXTSRAM_SIZE)
        {
            EMI_MPU_Unlock(i);
            for(j=0;j<EMI_MPU_DOMAIN_NUM;j++)
            {
                EMI_MPU_Set_Region_APC(i, j, EMI_MPU_NOPORTECT);
            }
            EMI_MPU_Set_Region_Address(i, protect_start_addr, protect_start_addr+protect_range);
            for(j=0;j<EMI_MPU_DOMAIN_NUM;j++)
            {
                EMI_MPU_Set_Region_APC(i, j, EMI_MPU_SR_NSNA);
            }
            EMI_MPU_Lock(i);
        }
        ASSERT(i<=EMI_MPU_REGION_NUM);
    }
#endif /* EMI_MPU_ENABLE */
    
    ARM_MPU_SETTING.magic_pattern = MPU_DUMP_MAGIC;
