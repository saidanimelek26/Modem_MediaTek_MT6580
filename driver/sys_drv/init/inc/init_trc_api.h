...
#define CCCI_BOOT_TS(trc_id) \
    if((BOOT_##trc_id > BOOT_INIT_CCCIHS1) && (BOOT_##trc_id <= BOOT_INIT_CCCIHS2)) \
        ccci_boottrc_write_log_sharemem((kal_uint32)BOOT_##trc_id, MCU_PMU_ReadCounter(PMU_CYCLE_COUNTER)); 

#define TASK_Trace(task_index) \
        ccci_boottrc_write_log_sharemem((kal_uint32)task_index, MCU_PMU_ReadCounter(PMU_CYCLE_COUNTER));
