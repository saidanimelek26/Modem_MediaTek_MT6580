...
//	p_still_size_param = g_USBVideo.camera_param->usbvideo_get_still_size_info(&still_size_num);
	...
	for(index = 0; index < still_size_num; index++)
	{
		...
		{
			EXT_ASSERT(0, (kal_uint32)video_req->buffer_size, index, p_still_size_param[index].max_frame_size);
		}
	}
...
//		DclSGPT_Control((DCL_HANDLE)(g_USBVideo.video_control.timer_handle), SGPT_CMD_STOP, 0);
	...
	else
	{
		...
		if(g_USBVideo.video_control.video_buffer_full == KAL_TRUE)
		{
			...
			{
				EXT_ASSERT(0, (kal_uint32)g_USBVideo.video_control.video_buffer_read_index,
							(kal_uint32)g_USBVideo.video_control.video_buffer_write_index, 0);
			}
...
//	p_video_size_param = g_USBVideo.camera_param->usbvideo_get_video_size_info(&video_size_num);
	...
	if(USB_DMA_Get_Run_Status(g_USBVideo.txpipe->byEP, USB_EP_TX_DIR) == KAL_TRUE)
	{
		...
		USB_GPTI_StartItem(g_USBVideo.video_control.timer_handle, USBVIDEO_PRE_PAUSE_TIMEOUT, USBVideo_PrePause_Timeout, NULL);
//		start.u2Tick = USBVIDEO_PRE_PAUSE_TIMEOUT;
//		start.pfCallback = USBVideo_PrePause_Timeout;
//		start.vPara = NULL;
//		DclSGPT_Control((DCL_HANDLE)(g_USBVideo.video_control.timer_handle), SGPT_CMD_START, (DCL_CTRL_DATA_T*)&start);
		bSendMsg = KAL_FALSE;
	}
	else
	{
...
//		DclSGPT_Control((DCL_HANDLE)(g_USBVideo.video_control.timer_handle), SGPT_CMD_STOP, 0);
	...
	else if(g_USBVideo.state == USBVIDEO_STATE_PAUSE)
	{
		...
		{	
			USB_GPTI_StartItem(g_USBVideo.video_control.timer_handle, USBVIDEO_INCALL_JPEG_TIMEOUT, USBVideo_Incall_Timeout, NULL);
