...
#define USB_MAX_INTERFACE			8  /* interface number */
#else
...
#if defined(__USB_MULTIPLE_COMPORT_SUPPORT__)
#if defined (__USB_MODEM_CARD_SUPPORT__)
#define USB_MAX_EP_BULK_TX		8  /* max bulk in ep number */
#define USB_MAX_EP_BULK_RX		6  /* max bulk in ep number */
#else
...
// conf :  : must be the end of descriptor parameter
	...
	const USB_DEVICE_PARAM	*device_param;
		...
		kal_uint8			morph_cfg_num;
		kal_uint8			morph_interface_num[USB_MAX_CONFIG];
#endif	
} Usb_Device;
