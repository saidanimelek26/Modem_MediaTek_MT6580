...
#include "kal_public_defs.h" //MSBB change #include "stack_config.h"
	...
	TRC_MSG(USB_PWNDWN, "USB wakes up by PWNDWN EINT")
	TRC_MSG(USB_IS_RESUME, "USB is resume")
	TRC_MSG(USB_IS_REMOTE_WAKEUP, "USB is remote wakeup")
...
//	TRC_MSG(USBACM_TX_CLEAR_UNDERRUN, "USB Clear TX UnderRun by USB port %d")
	...
	TRC_MSG(USBACM_FLC_HIGHSPEED_FLUSH, "USB control pipe flush data")
	...
	TRC_MSG(PC_OS_IS_MAC,   "PC OS is MAC")
	TRC_MSG(PC_OS_IS_LINUX,   "PC OS is Linux")
	TRC_MSG(PC_OS_IS_WIN,   "PC OS is Windows")
	TRC_MSG(USB_ECM_CONFIG,   "USB configure as ECM")
	TRC_MSG(USB_NVRAM_VALUE,   "USB read nvram value : %x")	
	TRC_MSG(USB_MBIM_CONFIG,  "USB configure as MBIM")
	TRC_MSG(USB_CBW_DATA, "USB CBW data1 %x, data2 %x")
	TRC_MSG(USB_ECM_GET_POWER_FILTER,"USB ECM set power filter index %x, length %d")				
	TRC_MSG(USB_ECM_POWER_FILTER,"USB ECM get power filter %x")
	TRC_MSG(USB_ECM_SET_PACKET_FILTER,"USB ECM set packet filter %x")			
	TRC_MSG(USB_ECM_STATIC,"USB ECM get static %x")
	TRC_MSG(USB_ECM_ADDRESS,"USB ECM MAC address: %x")
	TRC_MSG(USB_ECM_CTRL_NOTIFY, "USB COMPORT: Intr Notify to USB port %d")
