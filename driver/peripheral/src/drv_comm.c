...
void Drv_Init_Phase1(void)
{
  ...
  DclFGPT_Initialize();
   ...
   INT_Trace_Enter(INIT_DRV1_DMA, 1);
   DMA_Ini();
   INT_Trace_Exit(INIT_DRV1_DMA, 1);
   ...
   INT_Trace_Enter(INIT_DRV1_ADC, 1);
   DclHADC_Initialize( );
   INT_Trace_Exit(INIT_DRV1_ADC, 1);
...
//#ifndef DRV_LCD_NOT_EXIST
//   INT_Trace_Enter(INIT_DRV1_LCD, 1);
//   lcd_system_init();
//   INT_Trace_Exit(INIT_DRV1_LCD, 1);
//#endif /*DRV_LCD_NOT_EXIST*/
#ifndef DRV_RTC_NOT_EXIST
#ifdef DRV_RTC_HW_CALI
   INT_Trace_Enter(INIT_DRV1_RTCHW, 1);
   DclRTC_Control(rtc_handler, RTC_CMD_HW_INIT, (DCL_CTRL_DATA_T *)NULL);
   INT_Trace_Exit(INIT_DRV1_RTCHW, 1);
#endif
#endif /*DRV_RTC_NOT_EXIST*/
	DclPW_Initialize();
...
//#endif   /* __SWDBG_SUPPORT__ */
...
#ifdef __USB_COM_PORT_ENABLE__
   INT_Trace_Enter(INIT_DRV2_USB2UART, 1);
   ...
   INT_Trace_Exit(INIT_DRV2_USB2UART, 1);
#endif   /*__USB_COM_PORT_ENABLE__*/
...
#elif defined __CMUX_SUPPORT__
{
   ...
   INT_Trace_Enter(INIT_DRV2_EINTSWDBNC, 1);
   EINT_SW_Debounce_Init();
   INT_Trace_Exit(INIT_DRV2_EINTSWDBNC, 1);
   ...
   INT_Trace_Enter(INIT_DRV2_CHE, 1);
   che_hw_init();
   INT_Trace_Exit(INIT_DRV2_CHE, 1);

 	/* ISP/Camera initialize */
#if defined(ISP_SUPPORT)
    print_bootup_trace(INIT_DRV2_CIS_ENTER);
 	CalInit();
 	print_bootup_trace(INIT_DRV2_CIS_EXIT);
#endif
...
//#if (defined(_USE_SCCB_V2_DRIVER_))
   ...
   INT_Trace_Enter(INIT_DRV2_I2C, 1);
   DclSI2C_Initialize();
   INT_Trace_Exit(INIT_DRV2_I2C, 1);
#endif
