...
kal_uint8 U_GetUARTByteWithTimeOut(UART_PORT port, kal_uint8* ch, kal_uint32 timeout_value)
{
   ...
   while(timeout_value != 0)
   {
      ...
      if (LSR & UART_LSR_DR)
      {
         ...
         if( (kal_query_boot_mode()== FACTORY_BOOT && UARTPort[port].DCB.flowControl==fc_sw && UARTPort[port].ownerid==MOD_TST_READER)||
...
//workaround:  clear DLL and DLM before auto baudrate.
	...
	DRV_UART_WriteReg(UART_DLL(UART_BaseAddr[port]),1);  /*set to 1 in avoid of dirty byte sent when autobaudrate */
...
void U_SetDCBConfig(UART_PORT port, UARTDCBStruct *UART_Config, module_type ownerid)
{
   ...
   case fc_sw:
      ...
      {  
         ...
         if(kal_query_boot_mode() != FACTORY_BOOT || UARTPort[port].ownerid != MOD_TST_READER)
         {
            /*For AutoEscape*/
...
void UART_loopback_test(UART_PORT port,kal_bool enable)
{
   kal_uint16 tmp;
   kal_uint32 UART_BASE;
   /* Enable Loop Back test!! */
...
#if defined( DRV_UART_DMA_ENABLE_HANG_BUG)   /* move from uart_driverinit() to UART_hwinit  avoid of initing uart2 issue in MT6589*/
...
kal_bool U_Open(UART_PORT port, module_type ownerid)
{
   ...
   UARTPort[port].ownerid = ownerid;
       ...
       DMA_SleepDisable(UARTPort[port].Rx_DMA_Ch);
    #endif
...
DRV_UART_WriteReg(UART_RXDMA(UART_BASE),UART_TXRXDMA_OFF);
  ...
  #if defined(__DMA_LOW_POWER__)
     DMA_SleepEnable(UARTPort[port].Rx_DMA_Ch);
  #endif
}
...
U_Register_RX_cb(port, ownerid, UART_dafault_rx_cb);
 U_Close(port, (module_type)(MOD_UART1_HISR + port));
}
