...
#if defined(__ANDROID_MODEM__) || defined(DRV_GPIO_SP_ON)
#include "gpio_md.h"
#endif
#ifndef BL_ENABLE
...
//remove attribute because it is no use for gcc compiler(Qmei request)
void GPIO_WriteIO_FAST2(char data, kal_uint16 no, kal_uint16 remainder_shift)
{
 ...
 kal_uint16  get_6280_spec_index( kal_uint16	pin_num)
 	{   
#if !defined(DRV_GPIO_OFF)
#if defined (DRV_GPIO_6280_SPECIAL_SET_REG)
 ...
 kal_bool  Is_6280_SPEC_GPIO( kal_uint16 pin)
 	{
#if !defined(DRV_GPIO_OFF)
#if defined (DRV_GPIO_6280_SPECIAL_SET_REG)
 ...
 */
  ...
  char GPIO_MD_ReadIO(kal_uint16 port)
{ 
 #if defined(GPIO_DIN0)
	  kal_uint32 data = 0;
	  kal_uint16 no = 0;
	  kal_uint16 remainder = 0;
 #if defined(DRV_GPIO_REG_32BIT)
	  no = port/LENGTH_OF_32BIT_REG;
	  remainder = port % LENGTH_OF_32BIT_REG;
	  data = DRV_GPIO_Reg32(GPIO_DIN(no));
#elif defined(DRV_GPIO_REG_16BIT)  //DRV_GPIO_REG_32BIT
	  no = port/LENGTH_OF_16BIT_REG;
	  remainder = port % LENGTH_OF_16BIT_REG;
	  data = DRV_GPIO_Reg(GPIO_DIN(no));
#else
#error "!!!error:DRV_GPIO_REG_32BIT || DRV_GPIO_REG_16BIT !!!"
#endif  //DRV_GPIO_REG_32BIT
  ...
  void GPIO_MD_InitIO(kal_uint16 direction, kal_uint16 port)
 	{ 
 #if defined(GPIO_DIR0)
	  kal_uint16 no = 0;
	  kal_uint16 remainder = 0;
 #if defined(DRV_GPIO_REG_32BIT)
	  no = port/LENGTH_OF_32BIT_REG;
 ...
 */
  ...
  void GPIO_MD_WriteIO(kal_uint16 data, kal_uint16 port)
	{
#if defined(GPIO_DOUT0)
	  ...
	  no = port/LENGTH_OF_32BIT_REG;
	  remainder = port % LENGTH_OF_32BIT_REG;
	  if (data == GPIO_IO_HIGH)
		  DRV_GPIO_WriteReg32(GPIO_DOUT_SET(no),1<<remainder);
	  else
		  DRV_GPIO_WriteReg32(GPIO_DOUT_CLR(no),1<<remainder); 
...
void GPIO_MD_ModeSetup(kal_uint16 pin, kal_uint16 conf_dada)
   {
#if defined(GPIO_MODE0)
	  kal_uint16 no = 0;
	  kal_uint16 remainder = 0;
	  kal_uint32 savedMask=0;
#if defined(DRV_GPIO_REG_32BIT)
	  no = pin / GPIO_NUM_IN_32BIT_MODE_REG;
	  remainder = pin % GPIO_NUM_IN_32BIT_MODE_REG;
	  savedMask = SaveAndSetIRQMask();
	  DRV_GPIO_WriteReg32(GPIO_MODE_CLR(no),BITS_MASK_IN_32BIT_MODE_REG << (remainder*BITS_OF_ONE_MODE_IN_32BIT_REG));
	  DRV_GPIO_WriteReg32(GPIO_MODE_SET(no),conf_dada << (remainder*BITS_OF_ONE_MODE_IN_32BIT_REG));
      RestoreIRQMask(savedMask);
  
	#elif defined(DRV_GPIO_REG_16BIT)  //DRV_GPIO_REG_32BIT
	  no = pin / GPIO_NUM_IN_16BIT_MODE_REG;
	  remainder = pin % GPIO_NUM_IN_16BIT_MODE_REG;
	  
	  savedMask = SaveAndSetIRQMask();
	  DRV_GPIO_WriteReg(GPIO_MODE_CLR(no),BITS_MASK_IN_16BIT_MODE_REG << (remainder*BITS_OF_ONE_MODE_IN_16BIT_REG));
	  DRV_GPIO_WriteReg(GPIO_MODE_SET(no),conf_dada << (remainder*BITS_OF_ONE_MODE_IN_16BIT_REG));
  	  RestoreIRQMask(savedMask);
...
void GPIO_MD_PullenSetup(kal_uint16 pin, kal_bool enable)
 	{
#if defined(GPIO_PULLEN0)//mt6572 will not provide sucn function becasue the spec changed too much
	   kal_uint16 no = 0;
	   kal_uint16 remainder = 0;
	#if defined(DRV_GPIO_REG_32BIT)
	  no = pin / LENGTH_OF_32BIT_REG;
	  remainder = pin % LENGTH_OF_32BIT_REG;
	  if (enable == GPIO_PULL_ENABLE)
		  DRV_GPIO_WriteReg32(GPIO_PULLEN_SET(no),1 << remainder);
	  else
		  DRV_GPIO_WriteReg32(GPIO_PULLEN_CLR(no),1 << remainder);
	#elif defined(DRV_GPIO_REG_16BIT)  //DRV_GPIO_REG_32BIT
	  no = pin / LENGTH_OF_16BIT_REG;
	  remainder = pin % LENGTH_OF_16BIT_REG;
	  if (enable == GPIO_PULL_ENABLE)
		  DRV_GPIO_WriteReg(GPIO_PULLEN_SET(no),1 << remainder);
	 else
		  DRV_GPIO_WriteReg(GPIO_PULLEN_CLR(no),1 << remainder);
...
void GPIO_MD_PullSelSetup(kal_uint16 pin, kal_bool pull_up)
   	{
#if defined(GPIO_PULLSEL0)//mt6572 will not provide sucn function becasue the spec changed too much
			  kal_uint16 no = 0;
			  kal_uint16 remainder = 0;
	#if defined(DRV_GPIO_REG_32BIT)
			 no = pin / LENGTH_OF_32BIT_REG;
			 remainder = pin % LENGTH_OF_32BIT_REG;
			 if (pull_up == GPIO_PULL_HIGH)
				 DRV_GPIO_WriteReg32(GPIO_PULLSEL_SET(no),1 << remainder);
			 else
				 DRV_GPIO_WriteReg32(GPIO_PULLSEL_CLR(no),1 << remainder);
   #elif defined(DRV_GPIO_REG_16BIT)  //DRV_GPIO_REG_32BIT
			 no = pin / LENGTH_OF_16BIT_REG;
			 remainder = pin % LENGTH_OF_16BIT_REG;
			 if (pull_up == GPIO_PULL_HIGH)
				 DRV_GPIO_WriteReg(GPIO_PULLSEL_SET(no),1 << remainder);
			 else
				 DRV_GPIO_WriteReg(GPIO_PULLSEL_CLR(no),1 << remainder);
...
void GPIO_MD_DinvSetup(kal_uint16 pin, kal_bool enable)
	  {
	  #if 0  //dongming:AP gpio design changed every chip, and no user use this function, we don't need it !!!
/* under construction !*/
