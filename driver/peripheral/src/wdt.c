...
void WDT_SetValue(kal_uint16 value)
{
   ...
   ASSERT(tmp <= 0xff);
	...
	DRV_WDT_WriteReg32(WDT_LENGTH,(tmp | WDT_LENGTH_KEY));
#else
...
void WDT_Enable(kal_bool en)
{
	...
	#if defined(DRV_WDT_RETN_REG)
	DRV_WDT_WriteReg32(WDT_MODE,tmp);
	#else
...
void WDT_Set_IRQ_Mode(kal_bool mode)
{
#if defined(DRV_WDT_32K_MODE)
	kal_uint32 tmp;
	wdt_write_reg_delay();
	tmp = DRV_WDT_Reg(WDT_MODE);
	tmp |= WDT_MODE_KEY;
...
void WDT_init(void)
{
   ...
   #ifdef DRV_WDT_32K_MODE
   WDT_Set_IRQ_Mode(KAL_FALSE);
   #else
   WDT_Config(io_low,KAL_TRUE);
   #endif
   
   //#ifndef DRV_WDT_6575_REG
   #if !defined(DRV_WDT_6575_REG) && !defined(DRV_WDT_IRQ_MODE)
...
/*==========================================================*/
 //request by vend_wellken.chen for add debug time into ccci share memory
#if defined(__MODEM_CCCI_EXIST__)
  {
	  volatile kal_uint32 usc;
	  usc = ust_get_current_time();
	  ccci_excep_dbg_logging(CCCI_EXCEP_DBG_SW_WDT_TIME,(void*)&usc);
  }
#endif
/*===========================================================*/
