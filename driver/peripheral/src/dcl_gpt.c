...
DCL_HANDLE DclSGPT_Open(DCL_DEV eDev, DCL_FLAGS flags)
{
   ...
   if (DCL_GPT_CB==eDev)
   {
      ...
      for (i=0;i<32;i++)
   	...
   	{
   		if(is_gpt2_open == KAL_FALSE)
                 {
                         is_gpt2_open = KAL_TRUE;
   			return DCL_GPT_CB2_MAGIC_NUM |eDev;
                  }
		else
			return DCL_HANDLE_INVALID;
   	}
...
DCL_STATUS DclSGPT_Control(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	{
		gpt2_handle = DclHGPT_Open(GPT2, FLAGS_NONE);//SW GPT IS DIfferent from hw gpt
		switch(cmd)
		{
			case SGPT2_CMD_START:
			{
				DclHGPT_Control(gpt2_handle,HGPT_CMD_START,0);
                                DclHGPT_Close(gpt2_handle);
				return STATUS_OK;
			}
		
				//break; we can return in any case.
			
			case SGPT2_CMD_SET_COUNTVALUE:
			{
				SGPT2_CTRL_RESET_DATA_T   *prReset;
         		prReset = &(data->rHGPTReset);
         		DclHGPT_Control(gpt2_handle, HGPT_CMD_RESET, (HGPT_CTRL *)(prReset));
			DclHGPT_Close(gpt2_handle);	
                        return STATUS_OK;
			}
			...
			case SGPT2_CMD_STOP:
			{
		      	DclHGPT_Control(gpt2_handle, HGPT_CMD_STOP, 0);
                        DclHGPT_Close(gpt2_handle);
		      	return STATUS_OK;
			}
			
	
			case SGPT2_CMD_REGISTERCALLBACK:
			{
			
				SGPT2_CALLBACK_FUNC_T *prCallback;
				prCallback = &(data->gpt2_callback);
				DclHGPT_RegisterCallback(gpt2_handle, EVENT_HGPT_TIMEOUT, (PFN_DCL_CALLBACK)(prCallback->gpt2CallBack));
			        DclHGPT_Close(gpt2_handle);	
                                return STATUS_OK;
			}
			case SGPT2_CMD_SET_CLK:
			{
			
				SGPT2_CTRL_CLK_T   *prClock;
         		prClock = &(data->gpt2_clk);
		        DclHGPT_Control(gpt2_handle, HGPT_CMD_CLK, (HGPT_CTRL *)(prClock));
			DclHGPT_Close(gpt2_handle);		
                        return STATUS_OK;
			}
...
static void GPTI_StopItem(kal_uint8 module)
{
   ...
   //module &= ~(GPT_HANDLE_MAGIC_NUM);
   savedMask = SaveAndSetIRQMask();
   gpt_stop_array[gpt_stop_index++] = module;
   if(gpt_stop_index == 32)
        gpt_stop_index = 0;
   GPTTimer.status &= ~(1<<module);
   if(0==GPTTimer.status)
   {
