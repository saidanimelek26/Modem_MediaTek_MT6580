...
{\
	SIM_SetData(SIM_RETRY, SIM_RETRY_RXMASK, _RXRetry);\
}

#define SIM_SetTXRetry(_TXRetry) \
{\
	SIM_SetData(SIM_RETRY, SIM_RETRY_TXMASK, (_TXRetry<<8));\
}
...
#define SIM_SetIMP3(_IMP3)    SIM_WriteReg(SIM_IMP3, _IMP3)

#define SIM_SetCmdINS(_INS)   SIM_WriteReg(SIM_INS, _INS)

#define SIM_SetAtime(_ATIME)  SIM_WriteReg(SIM_ATIME, _ATIME)

#define SIM_SetDtime(_DTIME)  SIM_WriteReg(SIM_DTIME, _DTIME)

#define SIM_FIFO_Flush()      SIM_WriteReg(SIM_COUNT, 0x01)
...
{\
   ...
   {\
      ...
      SIM_WriteReg(SIM_IRQEN, SIM_IRQEN_SIMOFF);\
      SIM_FIFO_Flush();\
...
#define SIM_Active()    (SIM_WriteReg(SIM_CTRL, 0x0001))
...
{\
   ...
   {\
	...
	{\
	   kal_retrieve_eg_events(_SIMCARD.event,_flag,KAL_OR_CONSUME,&_event_group,0);\
	}	\
	sim_dump_error_line();\
}
...
{\
   ...
   SIM_WriteReg(SIM_IRQEN, SIM_IRQEN_ALLOFF);\
}

#define SIM_NotifyCARDisHALTEN()       SIM_SetBits(SIM_CONF, SIM_CONF_HALTEN)
#define SIM_T0CtrlEnable()             SIM_SetBits(SIM_CONF, SIM_CONF_T0EN)
#define SIM_T0CtrlDisable()            SIM_ClearBits(SIM_CONF, SIM_CONF_T0EN)
#define SIM_FlowCtrlEnable()             SIM_SetBits(SIM_CONF, SIM_CONF_HFEN) 
#define SIM_FlowCtrlDisable()            SIM_ClearBits(SIM_CONF, SIM_CONF_HFEN)

#define SIM_DisIntr(_Intr)            (SIM_ClearBits(SIM_IRQEN, _Intr))
...
{\
	if (_level == KAL_TRUE)\
	{\
		SIM_SetBits(SIM_CONF, SIM_CONF_CLKPO);\
		SIM_SetBits(SIM_CTRL, SIM_CTRL_HALT);\
	}\
	else\
	{\
	    SIM_ClearBits(SIM_CONF, SIM_CONF_CLKPO);\
	    SIM_SetBits(SIM_CTRL, SIM_CTRL_HALT);\
	}\
	DRVPDN_Enable(DRVPDN_CON1,DRVPDN_CON1_SIM,PDN_SIM);\
}
...
#define SIM_FIFO_GetLev()  (SIM_Reg(SIM_COUNT) & SIM_COUNT_MASK)
#define SIM_DisTOUTIntr()  SIM_ClearBits(SIM_IRQEN, SIM_IRQEN_TOUT)
#define SIM_OpenTOUTIntr()  SIM_SetBits(SIM_IRQEN, SIM_IRQEN_TOUT)
#define SIM_DisAllIntr()    SIM_WriteReg(SIM_IRQEN, SIM_IRQEN_ALLOFF)
#ifdef SIM_TOUT_REG_V2
#define SIM_SetTOUT(_TOUT)    \
{\
   SIM_ClearBits(SIM_CONF, SIM_CONF_TOUTEN);\
   if (_TOUT < 0xffffff)\
   	SIM_WriteReg32(SIM_TOUT, _TOUT);\
   else\
...
{\
   SIM_ClearBits(SIM_CONF, SIM_CONF_TOUTEN);\
   if (_TOUT < 0xffff)\
   	SIM_WriteReg(SIM_TOUT, _TOUT);\
   else\
   	SIM_WriteReg(SIM_TOUT, 0xffff);\
   SIM_Reg(SIM_STS);\
   SIM_SetBits(SIM_CONF, SIM_CONF_TOUTEN);\
}
#endif
