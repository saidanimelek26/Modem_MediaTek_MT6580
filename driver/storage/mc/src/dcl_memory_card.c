...
***************************************************************************/
/*SD related handle*/
#define DCL_SD_HANDLE_SD1	(MSDC_base|SD_MSDC1)
#if defined(__MSDC2_SD_MMC__)
#define DCL_SD_HANDLE_SD2	(MSDC2_base|SD_MSDC2)
#endif

#if defined(__SIM_PLUS__)
#define DCL_SD_HANDLE_SIMPLUS	(MSDC_base | SD_SIM)
#elif defined(DRV_MSDC_DUAL_TCARD_BY_SWITCH)
#define DCL_SD_HANDLE_SIMPLUS	(MSDC_base | SD_T_CARD_2)
#endif
#define DCL_SD_GET_SEL_FROM_HANDLE(a) ((sd_select_enum)(a & 0xF))

/*SDIO related handle*/
#define DCL_SDIO_HANDLE_SDIO1	(MSDC_base|SD_MSDC1)
#if defined(__MSDC2_SD_SDIO__)
#define DCL_SDIO_HANDLE_SDIO2	(MSDC2_base|SD_MSDC2)
#endif
#define DCL_SDIO_GET_SEL_FROM_HANDLE(a) ((sd_select_enum)(a & 0xF))

/*MS related handle*/
#define DCL_MS_HANDLE	(MSDC_base|0)
...
{
	IRQMask(irqMSDCCode);
	if(MSDC_Reg(addrReg_MSDC_PS) & MSDC_PS_PIN0)
	{
...
#if defined(DRV_MSDC_LATCH_MT6276_SERIES)
#else
	SD_SetRedDlt(MSDC_Blk[sel].msdc_RedDlt_forRd);
...
#if defined(DRV_MSDC_LATCH_MT6276_SERIES)
#else
	if(MSDC_RED_DLT_INVALID != MSDC_Blk[sel].msdc_RedDlt_forWr)
		SD_SetRedDlt(MSDC_Blk[sel].msdc_RedDlt_forWr);
#endif//#if defined(DRV_MSDC_LATCH_MT6276_SERIES)
#endif
...
DCL_HANDLE DclSD_Open(DCL_DEV dev, DCL_FLAGS flags)
{
	...
	if(flags & DCL_SD_FLAGS_DEVICE_CARD1){
		...
		kal_uint16	magic;
		get_MSDC_lock(&dclMsdcLock[SD_T_CARD_2]);
		magic = (sdResource[SD_T_CARD_2] & DCL_SD_RESOURCE_MAGIC_MAX);
			if(DCL_SD_RESOURCE_MAGIC_MAX == magic) /*if means 0x0FF0 achieved*/
				magic = DCL_SD_RESOURCE_MAGIC_UNIT;
			else
				magic += DCL_SD_RESOURCE_MAGIC_UNIT;
			
			sdResource[SD_T_CARD_2] = (DCL_SD_RESOURCE_OCCUPIED | magic);

			sdDbgRrc[SD_T_CARD_2].callerRetAddr = retAddr;
			sdDbgRrc[SD_T_CARD_2].ownerThdId = thdId;
