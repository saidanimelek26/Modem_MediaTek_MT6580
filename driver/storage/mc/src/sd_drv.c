...
// for read operation performance measurement
	...
	{		
        ...
        if (dclStatus == STATUS_ERROR_TIMEOUT ||
            ...
            tst_sys_trace("[MSDC]: Error recover(read cmd timeout)");
            ...
            tst_sys_trace("[MSDC]: Error recover(read crc or gpt timeout error)");
            
#if defined(DRV_MSDC_RETRY_DLT_IN_READ_ERR)
            //
            // [Programming guide for error handling]
            //
            // Every time an error occurred, we try all the DLT which smaller than clock factor,
            // Until DLT is over than clock factor, we reset the DLT count and slow down the clock rate.
            // Finally, we try the loop again.
            //
            ClkFactor = gMSDC_Handle->msdc_clock / gMSDC_Handle->op_clock;
            
            if ((gMSDC_Handle->msdc_RedDlt_forRd & 0xffff) < ClkFactor) {
                gMSDC_Handle->msdc_RedDlt_forRd++;
            }
            else 
#endif
            {
...
#if defined(DRV_MSDC_RETRY_DLT_IN_READ_ERR)
                gMSDC_Handle->msdc_RedDlt_forRd = 1;
#endif
            }        
        }
	} // end of if error
