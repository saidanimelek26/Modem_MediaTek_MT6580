...
/**************** WDT ******************/
   ...
   //WDT send WDT reset interrupt to AP side while AP side in sleep,it will can not receive the interrupt.So must send msg via CCCI to wake up AP side.
   //the bug will be fix on MT658*,beacuse WDT interrupt will be AP wake up source.
   
   //1.For USBlv2
   //2. because add 32bit retention register,SW reset and restart register also from 16bit to 32bit
#elif defined(DRV_WDT_6572_SERIES)
   #define DRV_WDT_BASIC_REG
   #define DRV_WDT_LENGTH_11BITS
   #define DRV_WDT_SWRST
   #define DRV_WDT_RETN_REG	
...
#elif defined(DRV_WDT_6582_SERIES)
   #define DRV_WDT_BASIC_REG
   #define DRV_WDT_LENGTH_11BITS
   #define DRV_WDT_SWRST
   #define DRV_WDT_RETN_REG	
   #define DRV_WDT_IRQ_MODE
   #define DRV_WDT_32K_MODE   //from 6572,wdt can config the ways interrupt sendded out. 32K  clock, or bus clock.
#elif defined(DRV_WDT_6592_SERIES)
   #define DRV_WDT_BASIC_REG
   #define DRV_WDT_LENGTH_11BITS
   #define DRV_WDT_SWRST
   #define DRV_WDT_RETN_REG	
   #define DRV_WDT_IRQ_MODE
   #define DRV_WDT_32K_MODE   //from 6572,wdt can config the ways interrupt sendded out. 32K  clock, or bus clock.
#elif defined(DRV_WDT_6571_SERIES)
   #define DRV_WDT_BASIC_REG
   #define DRV_WDT_LENGTH_11BITS
   #define DRV_WDT_SWRST
   #define DRV_WDT_RETN_REG	
   #define DRV_WDT_IRQ_MODE
   #define DRV_WDT_32K_MODE   //from 6572,wdt can config the ways interrupt sendded out. 32K  clock, or bus clock.
#elif defined(DRV_WDT_6580_SERIES)
   #define DRV_WDT_BASIC_REG
   #define DRV_WDT_LENGTH_11BITS
   #define DRV_WDT_SWRST
   #define DRV_WDT_RETN_REG	
