...
MCU_MK_IMAGE_CFG  := tools/MKImage/md1rom_hdr.cfg
MCU_MK_IMAGE_TOOL := tools/MKImage/mkimage20
...
ifneq ($(filter $(call Upper,$(strip $(PROJECT_NAME))),$(strip $(DEFAULT_CUSTOMER))),)
  SCATTERFILE = $(strip $(BUILD_SYSDIR))/scat$(strip $(PLATFORM)).txt
  ifeq ($(strip $(TEST_LOAD_TYPE)),L1S)
    SCATTERFILE := $(strip $(BUILD_SYSDIR))/scat$(strip $(PLATFORM))_L1S.txt
  endif
  ifeq ($(strip $(TEST_LOAD_TYPE)),BASIC)
    SCATTERFILE := $(strip $(BUILD_SYSDIR))/scat$(strip $(PLATFORM))_BASIC.txt
  endif
  ifeq ($(strip $(TEST_LOAD_TYPE)),UDVT)
    SCATTERFILE := $(strip $(BUILD_SYSDIR))/scat$(strip $(PLATFORM))_UDVT.txt
  endif
else
  SCATTERFILE = $(strip $(BUILD_SYSDIR))/scat$(strip $(PROJECT_NAME)).txt
  ifeq ($(strip $(TEST_LOAD_TYPE)),L1S)
    SCATTERFILE := $(strip $(BUILD_SYSDIR))/scat$(strip $(PROJECT_NAME))_L1S.txt
  endif
  ifeq ($(strip $(TEST_LOAD_TYPE)),BASIC)
    SCATTERFILE := $(strip $(BUILD_SYSDIR))/scat$(strip $(PROJECT_NAME))_BASIC.txt
  endif
  ifeq ($(strip $(TEST_LOAD_TYPE)),UDVT)
    SCATTERFILE := $(strip $(BUILD_SYSDIR))/scat$(strip $(PROJECT_NAME))_UDVT.txt
  endif
endif
