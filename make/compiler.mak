...
ifneq ($(filter MT6589, $(strip $(PLATFORM))),)
...
ifneq ($(filter MT6571 MT6572 MT6582 MT6592 MT6580 MT6570, $(strip $(PLATFORM))),)
ifeq ($(strip $(COMPILER)),GCC)
    PLATFORM_FLAGS  := -march=armv7-r -mcpu=cortex-r4 -mlittle-endian -Wa,-mimplicit-it=always -mabi=aapcs
    PLATFORM_AFLAGS  := -march=armv7-r -mcpu=cortex-r4 -mlittle-endian -mimplicit-it=always -mapcs-32
    OFLAGS          := -Os -fno-strict-aliasing -fno-common -fno-exceptions -ffunction-sections -fdata-sections
...
ifeq ($(strip $(armtoolver)),LSS794)
  ...
  ifneq ($(filter MT6280 MT6589MD1 MT6571 MT6572 MT6582 MT6592 MT6580 MT6570, $(strip $(PLATFORM))),)
...
ifeq ($(strip $(COMPILER)),RVCT)
  ...
  ifeq ($(strip $(PLATFORM)),TK6280)
    BL_AFLAGS  :=   --debug --littleend --cpu Cortex-R4 --no_unaligned_access --apcs /interwork
    AFLAGS     :=   --debug --littleend --cpu Cortex-R4 --no_unaligned_access --apcs /interwork
  endif
  ifeq ($(strip $(PLATFORM)),MT6280)
    ifeq ($(strip $(COMPILER)),GCC)
      BL_AFLAGS  :=   -g --littleend --cpu Cortex-R4 --thumb ¡Vmno-unaligned-access
      AFLAGS     :=   -g --littleend --cpu Cortex-R4 --thumb ¡Vmno-unaligned-access
    else
      BL_AFLAGS  :=   --debug --littleend --cpu Cortex-R4 --no_unaligned_access --apcs /interwork
      AFLAGS     :=   --debug --littleend --cpu Cortex-R4 --no_unaligned_access --apcs /interwork
    endif  
  ...
  ifneq ($(filter MT6575 MT6577, $(strip $(PLATFORM))),)
    ifeq ($(strip $(COMPILER)),GCC)
      BL_AFLAGS  :=   -g --littleend -mcpu=arm1176jz-s --thumb ¡Vmno-unaligned-access
      AFLAGS     :=   -g --littleend -mcpu=arm1176jz-s --thumb ¡Vmno-unaligned-access
    else
...
ifeq ($(strip $(COMPILER)),GCC)
  WRAP_LIST = --wrap=memcpy --wrap=memset --wrap=memclr --wrap=strcmp --wrap=sprintf  --wrap=vsprintf --wrap=snprintf --wrap=vsnprintf --wrap=sscanf --wrap=vsscanf \
	--wrap=raise --wrap=__aeabi_div0 --wrap=gmtime_r --wrap=setlocale --wrap=on_exit --wrap=getenv --wrap=clock --wrap=time --wrap=puts \
