...
ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
  ...
  ifeq ($(strip $(call Upper,$(UE_SIMULATOR))),TRUE)
    TMD_TO_C_UESIM := $(foreach tmd_file,$(strip $(TMD_FILES)),$(CGEN_L1_OUTDIR)/$(notdir $(basename $(tmd_file))).c)
    MODIS_TOUCH_FILE += $(TMD_TO_C_UESIM)
  endif
else
  TMD_FILES :=
...
ifeq ($(strip $(TEST_LOAD_TYPE)),L1S)
  ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
    CODEGEN_RESULT = $(strip $(MODISBUILD_TST_DB))/l1validation_db/BPLVInfo
  endif
else
...
ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
  ifneq ($(strip $(TEST_LOAD_TYPE)),L1S)
    ...
    CODEGEN_DATABASE_OUT = $(strip $(MODISBUILD_TST_DB))/l1validation_db/BPLVInfo
  endif
else
...
ifneq ($(filter __MOD_NVRAM__,$(strip $(COM_DEFS))),)
   ...
   ifeq ($(strip $(OS_VERSION)),MSWin32)
	...
	@perl tools/pack_dep_gcc.pl $(RULESDIR_TARGET)/nvram_auto_gen.dep $(PROJDIR)/nvram_auto_gen/debug
...
ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
	...
	@$(ECHO) -n  "$@: tools/CreateEmptyTrcGenFiles.exe $(strip $(TSTDIR))/database/TrcGen.lst $(NEED_CHECK_DEPEND_LIST)" >$@
	@$(ECHO) -n  "$@" >>$(CODE_GEN_LST)
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_E,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(ECHO) -n "Generate RD3 Base Information ..."
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
	@$(ECHO) -n "$(CGEN) -ap3 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/libParseDbModem.db $(MODISBUILD_TST_DB)/enumFile $(strip $(PLATFORM))_$(strip $(CHIP_VER)) $(strip $(VERNO))" >>$(CGEN_CFG_MODEM)
else
	@echo $(CGEN) -ap3 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/libParseDbModem.db $(MODISBUILD_TST_DB)/enumFile MoDIS $(strip $(VERNO)) >>$(CGEN_CFG_MODEM)
endif
...
ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
  ...
  ifeq ($(strip $(RUN_CGEN_L1)),TRUE)
	...
	@$(FOR) $(DECLARE_VAR)i $(IN) $(foreach tmd_file,$(filter-out CuStOm_StArT_AST CuStOm_EnD_AST,$(strip $(TMD_FILES))), $(CGEN_L1_OUTDIR)/$(notdir $(basename $(tmd_file))).c $(BUILD_TSTDIR)/include/$(notdir $(basename $(tmd_file))).h) $(DO) (echo $(PRE_VAR)i >>$(CODE_GEN_LST)) $(DONE)
    endif
  ...
  ifneq ($(strip $(AST_SUPPORT)),NONE)
	@$(IF_NOT_EXIST) $(BUILD_TSTDIR)/src $(THEN) \
		$(MKDIR) $(BUILD_TSTDIR)/src $(CMD_END)
    ifneq ($(strip $(MODIS_CONFIG)),TRUE)
	@$(ECHO) -n "$(CGEN) -L1 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) L1_CATCHER $(BUILD_TSTDIR)/src/TrcMod.c $(BUILD_TSTDIR)/include/TrcMod.h $(AST_TMD_FILES) -OutputCPath-$(CGEN_L1_OUTDIR)/" >>$(CGEN_CFG_MODEM)
	@$(STRCMPEX) abc abc e $@ $@: $(filter-out CuStOm_StArT_AST CuStOm_EnD_AST,$(strip $(AST_TMD_FILES)))
	@$(FOR) $(DECLARE_VAR)i $(IN) $(foreach tmd_file,$(filter-out CuStOm_StArT_AST CuStOm_EnD_AST,$(strip $(AST_TMD_FILES))), $(CGEN_L1_OUTDIR)/$(notdir $(basename $(tmd_file))).c $(BUILD_TSTDIR)/include/$(notdir $(basename $(tmd_file))).h) $(DO) (echo $(PRE_VAR)i >>$(CODE_GEN_LST)) $(DONE)
	@$(ECHO) -n "$(BUILD_TSTDIR)/src/TrcMod.c" >>$(CODE_GEN_LST)
	@$(ECHO) -n "$(BUILD_TSTDIR)/include/TrcMod.h" >>$(CODE_GEN_LST)
    else
      # workaround for -puremodis, because some L1 headers are always included
      ifeq ($(strip $(OS_VERSION)),MSWin32)
	@for %%i in ($(BUILD_TSTDIR)/include/TrcMod.h $(foreach tmd_file,$(filter-out CuStOm_StArT_AST CuStOm_EnD_AST,$(strip $(AST_TMD_FILES))),$(subst /,\,$(BUILD_TSTDIR)/include/$(notdir $(basename $(tmd_file)).h)))) do (if not exist %%i copy /y NUL %%i >NUL)
      else
	@touch -a $(BUILD_TSTDIR)/include/TrcMod.h $(foreach tmd_file,$(filter-out CuStOm_StArT_AST CuStOm_EnD_AST,$(strip $(AST_TMD_FILES))),$(BUILD_TSTDIR)/include/$(notdir $(basename $(tmd_file)).h))
      endif
    endif
  endif
  endif
...
$(MODIS_RULESDIR)/codegen_dep/cgen_gv.det: $(MODIS_RULESDIR)/codegen_dep/cgen_c.det $(MODIS_RULESDIR)/codegen_dep/gv_db.det
...
$(MODIS_RULESDIR)/codegen_dep/cgen_l1v.det: $(MODIS_RULESDIR)/codegen_dep/cgen_c.det $(MODIS_RULESDIR)/codegen_dep/l1v_db.det
...
$(MODIS_RULESDIR)/codegen_dep/cgen_un.det: $(MODIS_RULESDIR)/codegen_dep/cgen_c.det $(MODIS_RULESDIR)/codegen_dep/unionTag_parse.det
...
sysgen_conf: emigen obj_list_gen
   # mbis time probe
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_S,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
	...
	@$(IF_EXIST) $(FIXPATH)/tools/sys_auto_gen.pl $(THEN) \
		((perl $(FIXPATH)/tools/sys_auto_gen.pl $(SCATTERFILE) $(BL_SCATTERFILE) "$(strip $(PROJECT_MAKEFILE))" $(strip $(BUILD_SYSDIR)) $(strip $(NEED_BUILD_BOOTLOADER)) $(strip $(INSIDE_MTK)) $(strip $(DUMMY_LIS)) $(strip $(FACTORY_BIN_FOLDER))/FACTORY_ROM $(LIBDIR_TEMP) $(strip $(CC))> $(strip $(COMPLOGDIR))/sysgen.log) \
		$(WITH) \
...
# mbis time probe
  ...
  ifneq ($(strip $(MODIS_CONFIG)),TRUE)
		@$(IF_EXIST) service/config/src/hal/syscomp_config.c $(THEN)\
			...
			(perl tools/sys_mem_gen.pl $(strip $(TMPDIR))/~syscomp_config.o $(strip $(BUILD_SYSDIR)) $(MODIS_CONFIG) $(strip $(COMPLOGDIR))>>$(strip $(COMPLOGDIR))/sys_mem_gen.log $(WITH) \
		...
		@$(IF_EXIST) service/config/src/hal/syscomp_config.c $(THEN)\
			...
			(perl tools/sys_mem_gen.pl $(strip $(TMPDIR))/~syscomp_config.o $(strip $(BUILD_SYSDIR)) $(MODIS_CONFIG) $(strip $(MODISLOGDIR))>>$(strip $(MODISLOGDIR))/sys_mem_gen.log $(WITH) \
