...
# *************************************************************************
#  AAPMC for File System Configuration
# *************************************************************************
...
ifeq ($(strip $(COMPILER)),GCC)
	@$(IF_EXIST) $(FIXPATH)/tools/MemoryUtility/memquery.pl $(THEN) \
		perl $(FIXPATH)/tools/MemoryUtility/memquery.pl 1 $(strip $(TMPDIR))/~memquery.tmp $(SCATTERFILE) "$(strip $(PROJECT_MAKEFILE))" $(strip $(TARGDIR))/$(TARGNAME).map $(strip $(COMPLOGDIR)) \
	$(CMD_END)
else
	@$(IF_EXIST) $(FIXPATH)/tools/MemoryUtility/memquery.pl $(THEN) \
		perl $(FIXPATH)/tools/MemoryUtility/memquery.pl 2 $(strip $(TMPDIR))/~memquery.tmp $(SCATTERFILE) "$(strip $(PROJECT_MAKEFILE))" $(strip $(TARGDIR))/$(TARGNAME).lis $(strip $(COMPLOGDIR)) \
	$(CMD_END)
endif
