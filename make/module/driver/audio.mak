...
SRC_LIST += $(strip $(AUDFOLDER))/am.c \
            ...
            $(strip $(AUDFOLDER))/tone2.c \
            $(strip $(AUDFOLDER))/tone_loopback_rec.c \
            $(strip $(AUDFOLDER))/vm.c \
            driver/audio/src/audio_create.c

# AFE part
AFE_SPLIT_DRIVER_PLATFORM  = MT6280 MT6589 MT6572 MT6582 MT6592 MT6571 MT6580 MT6570
ifneq ($(filter $(strip $(MODULE_DEFS)), $(AFE_SPLIT_DRIVER_PLATFORM)),)
   ...
   ifeq ($(strip $(PLATFORM)),MT6580)
     SRC_LIST += $(strip $(AUDFOLDER))/afes/afe_6580.c
   endif
   
   ifeq ($(strip $(PLATFORM)),MT6570)
     SRC_LIST += $(strip $(AUDFOLDER))/afes/afe_6580.c
   endif
...
ifneq ($(filter __DATA_CARD_SPEECH__, $(strip $(MODULE_DEFS))),)
   ...
   SRC_LIST +=  $(strip $(AUDFOLDER))/sp_daca.c                                                         
endif
...
ifneq ($(filter BGSND_ENABLE, $(strip $(MODULE_DEFS))),)
  ifeq ($(strip $(AUD_SOLUTION)),V1)
    SRC_LIST += $(strip $(AUDFOLDER))/bgSnd.c
  endif
  ifeq ($(strip $(AUD_SOLUTION)),V2)
